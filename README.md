...
> **상태: 보류 (미착수).** 기록을 유도하려고 홈에 **육성 요소**(캐릭터·나무·섬)를 넣는 방향을
...
컨트롤러 27개·화면 60개에 소급**해야 한다. 그래서 경로만 먼저 깔고 값은 열어뒀다.
특히 운동 v2가 미착수라, 새로 짜는 코드는 처음부터 플랜을 인지한 채로 태어난다.
...
| 확장 | 하단 탭 재구성 — 운동/식단 분리(5탭) + FAB 제거, 홈 오늘 칩이 그 역할 대체 — PLAN.md | 🚧 화면설계 |
