...
Dubly는 **관계(Relation) 기반 커플 라이프 공유 앱**입니다. 커플은 운동·식단·맛집·여행·일상을
...
> **이름 표기 규칙**: 사람이 읽는 곳(문서·주석·UI)은 **Dubly**, 앱 식별자(`com.doubly.app` 등)는
> **`doubly` 유지**, 기계가 읽는 레거시 내부 식별자는 **`fitto` 유지**입니다 — Java 패키지
> `com.fitto`, 설정 prefix `fitto.*`, 스토리지 키 `fitto.accessToken`, Railway 호스트
> `fitto-production…`, Cloudinary preset `fitto_unsigned` 등. 이들은 배포된 인프라·기존 사용자
> 세션과 짝이 맞아야 해서 **문자열만 바꾸면 깨집니다**. 자세한 목록은
> [이름 전환 범위](#이름-전환-범위--dubly-로-바꿀-것과-두어야-할-것)와
> [docs/BRAND_RENAME_DUBLY.md](docs/BRAND_RENAME_DUBLY.md) 참고.
...
## 이름 전환 범위 — Dubly 로 바꿀 것과 두어야 할 것

프로젝트는 `Fitto` → `Doubly` → `Dubly` 순으로 두 차례 리브랜딩됐지만, 코드·인프라 식별자까지
한꺼번에 바꾸면 **배포된 서비스와 기존 사용자 세션이 깨집니다.** 그래서 표기(사람이 읽는 것)와
식별자(기계가 읽는 것)를 항상 분리해서 관리하며, 지금은 3계층입니다.

| 계층 | 값 | 대상 |
| --- | --- | --- |
| 브랜드 표기 | **Dubly** | 문서 제목·주석·UI 문구, 스토어 표시 이름 |
| 앱 식별자 | `doubly` | `com.doubly.app`(iOS/Android), Firebase `doubly-5f40c`, Sentry `doubly` 프로젝트, 딥링크 `doubly://`, `doubly.*` 스토리지 키 — **스토어에 이미 등록 완료돼 영구 고정** |
| 레거시 내부 식별자 | `fitto` | Java 패키지 `com.fitto`, 설정 prefix `fitto.*`, Railway 호스트 `fitto-production…`, Cloudinary preset `fitto_unsigned` 등 |

`Doubly → Dubly` 전환 배경, 실제 변경 파일 목록(2026-08-21 1차 완료분)은
[docs/BRAND_RENAME_DUBLY.md](docs/BRAND_RENAME_DUBLY.md) 참고.

### ✅ Dubly 로 바꾼다 — 사람이 읽는 곳
...
| `docs/EAS_BUILD.md` | "홈 화면에 Dubly 아이콘 생성" |
| `docker-compose.yml` · `backend/Dockerfile` | 최상단 주석 |
| `frontend/src/theme/*.ts` · `constants/theme.ts` | "Dubly 디자인 토큰" 주석 |

### ⛔ 두어야 한다 — 기계가 읽는 식별자

| 대상 | 왜 두는가 | 바꾸려면 |
| --- | --- | --- |
| `com.doubly.app` · Firebase `doubly-5f40c` · Sentry `doubly` · 딥링크 `doubly://` · `doubly.*` 스토리지 키 | **스토어에 이미 등록 완료.** 바꾸면 기존 설치·리뷰·랭킹 전부 소실, 로그인 세션·크래시 로그 이력 단절 | 사실상 불가 |
...
> 요약: **표기(Dubly)·앱 식별자(doubly)·레거시 식별자(fitto) 3계층을 분리**해서 관리합니다.
...
짐워크는 개인 운동일지 앱이고, Dubly 는 그 결과를 **둘이 나누는** 앱입니다.
따라서 세트 로그·볼륨·1RM 은 *수단*이고, 목적은 **볼륨·PR·성장을 커플 피드·대결·스트릭에 태우는 것**입니다.
이 구분이 흐려지면 Dubly 가 존재할 이유가 사라집니다.
...
- 세션이 서버에 살아있으면 **앱이 죽어도 이어서** 할 수 있고, **다음 세트 무게를 추천**할 수 있고,
  상대에게 **"지금 운동 중"** 을 띄울 수 있습니다 (마지막 항목은 짐워크에 없는 Dubly 고유 기능)
...
로고(`DoublyLogo.tsx`)가 원 2개 겹침이고, 앱 이름이 "Dubly", 슬로건이 "둘이라서, 두 배로"입니다.
...
앱 이름이 "Dubly", 슬로건이 "둘이라서, 두 배로", 로고가 두 덩굴이 얽힌 하트,
...
| 항목 | 값 | 바꾸는 비용 |
| --- | --- | --- |
| `ios.bundleIdentifier` / `android.package` | `com.doubly.app` | **스토어 등록 완료로 이미 확정, 변경 불가** |
| 앱 이름 / slug | `Dubly` / `doubly` | 이름(표시값)은 저렴 — 실제로 2026-08-21 `Doubly → Dubly` 전환 완료. slug 는 앱 식별자라 `doubly` 유지 |
| 슬로건 "둘이라서, 두 배로" | 스플래시·로그인·MY·PWA 설명 **4곳** | 저렴 |
| `Dubly` 문자열 | **28개 파일** (구 `Doubly`) | 저렴 |
| 로고 | 두 덩굴이 얽혀 이루는 하트 | 저렴 |
| Duo Color System | 나·상대·함께 3색 | N인이면 인덱스 팔레트 필요 |

### ⚠️ 되돌릴 수 없는 것은 번들 ID 하나뿐이다

`com.doubly.app` 은 **이미 스토어에 등록 완료돼 더 이상 바꿀 수 없다.** 바꾸면 **기존 앱과
완전히 다른 앱**이 되어 사용자·리뷰·순위를 전부 잃는다. 나머지(표시 이름·슬로건·로고·문자열)는
번들 ID와 달리 지금도 언제든 저렴하게 바꿀 수 있다 — `Doubly → Dubly` 표시명 전환이 그 실증이다.

**즉 ② 개명(번들 ID·slug 교체)의 창은 이미 닫혔다.** 남은 선택은 ①/③ 중에서, 표시 요소(이름·
슬로건·로고)만으로 방향을 조정하는 것뿐이다.

### 선택지

| 안 | 내용 | 비용 |
| --- | --- | --- |
| **① Dubly 유지 + 그룹은 부가 기능** | 슬로건만 "**함께**라서, 두 배로" 로 한 단어 교체. "두 배"가 인원수가 아니라 *함께 해서 생기는 효과*로 읽힌다. 로고(얽힌 덩굴)도 "연결"로 계속 통한다 | 최소 |
| ② 개명 — 그룹을 메인으로 | 번들 ID·이름·로고·슬로건 전부 교체. 제품 재정의 | 큼. **스토어 등록 완료로 이미 불가** |
...
1. 슬로건만 "함께라서, 두 배로" 로 교체 — 이름(표시값)·번들 ID 는 유지
...
4. 데이터가 "그룹이 메인" 이라고 말해도 ② 는 번들 ID 문제로 이미 선택지에서 빠졌으므로,
   그때는 ①(표시 요소만 재조정)의 폭을 얼마나 넓힐지를 재검토한다

②는 애초에 근거 유무와 무관하게 **번들 ID가 이미 고정돼 실행 불가능**하다. ③ 은 이름 때문에
기능을 포기하는 것이라 더 아깝다.

> **판단 기한**: 이미 지났다 — 스토어 등록이 완료되어 ② 의 창은 닫혔다. 이제는 ①/③ 중 표시
> 요소로만 판단하면 된다.
...
| 그룹(보류) | 포인트 쇼핑몰 연계 | ⏸ 보류 |
| 결정 필요 | 브랜드 — 그룹 도입 시 이름 유지 여부 (**번들 ID는 스토어 등록 완료로 이미 고정, 표시 요소만 검토 가능**) — 위 [브랜드 절](#브랜드-결정--그룹이-들어오면-dubly-가-맞나-미결) | ❓ 미결 |
...
| 확장 | 럽슐랭 클린식/치팅데이 구분(dietTag) 기능 제거 — 위시리스트 목적과 안 맞는다는 판단, 스키마(V64)·필터·핀 색상·범례까지 정리 | ✗ 드롭 |
| 확장 | 럽슐랭 솔로 트랙 "내 픽/상대 픽" — 한쪽만 4점 이상 매긴 곳도 인정(`SoloPickBadge`), 가이드 상단 가로 스크롤 섹션 | ✅ 완료 |
...
| 통화 · 영상통화 | 2단계 — 프론트 Stream SDK 통합 + 전역 `CallOverlay`(포그라운드 통화 중 UI) + 채팅 헤더 통화 버튼 — PLAN.md | ✅ 완료 — 실기기 2대·실계정으로 음성/영상 통화 실제 연결까지 검증 완료(2026-08-25) |
| 통화 · 영상통화 | 부재중 통화 채팅 카드("다시 걸기") + 30초 무응답 자동 판정 — 네이티브 벨 웨이크업 대체 경로, PLAN.md | ✅ 완료 — `CallSessionSweeper` 실기기 검증 완료 |
| 통화 · 영상통화 | 부재중 통화 배지 — 채팅 탭 아이콘(커플 계정은 `ChatScreen`을 거치지 않고 바로 `ChatRoom`으로 이동해 원래 배지 자리를 볼 수 없어서 이동) | ✅ 완료 |
| 통화 · 영상통화(버그 수정) | Stream 토큰 서명 알고리즘 HS256 미명시 — jjwt 가 키 길이(프로덕션 시크릿 64바이트)로 HS512 를 자동 선택해 통화가 전량 실패하던 문제 | ✅ 수정 완료(2026-08-25) |
| 통화 · 영상통화(버그 수정) | 음성통화 발신 시 Stream 400 — `settings_override.video` 에 `target_resolution` 누락 | ✅ 수정 완료(2026-08-25) |
| 통화 · 영상통화(선택) | 네이티브 벨 웨이크업 — iOS(PushKit/CallKit, `@stream-io/react-native-callingx`) | ✅ 완료(실기기 검증 대기) — Android(고우선순위 FCM)는 여전히 보류, PLAN.md |
| 알림 인프라 | 알림 탭 딥링크 — 푸시 `data.link` → `linking.ts` 경로 맵 재사용(콜드 스타트 포함) | ✅ 완료 |
| 알림 인프라 | 카테고리별 수신 설정 (채팅·기념일·상대 활동·리마인드, V58) + OS 권한 거부 시 시스템 설정 안내 | ✅ 완료 |
| 알림 인프라 | 재방문 리마인드 3종 (21시 KST 단일 스케줄러 — 스트릭 위기·질문 미답변·혼자 가입자 D+1/D+3, 사람당 하루 1건) | ✅ 완료 |
| 커플 루프 | 오늘의 질문 콘텐츠 30 → 215개 (`QuestionCatalog`) | ✅ 완료 |
| 커플 루프 | 응원 리액션을 피드 전 타입으로 (운동·식단·맛집 방문 카드, V60) | ✅ 완료 |
| 커플 루프 | 기념일 D-7·D-1 사전 푸시 (D-7 은 기념일·생일만) | ✅ 완료 |
| 커플 루프 | 스트릭 마일스톤 축하 7·30·100일 (커플은 양쪽 푸시 + `STREAK_CARD` 채팅 카드) | ✅ 완료 |
| 커플 루프 | 대결 종료 자동 판정 (09:30 KST — 승자 확정 저장 + 결과 푸시, V59) | ✅ 완료 |
| 운동 v2 | 운동 세션 로컬 복구 (10초 스냅샷 + 백그라운드 진입 시 저장 → "이어서 하기") — 서버 세션 전까지의 선행 단계 | ✅ 완료 |
| 운동 v2 | 운동 기록 상세 화면 (세트별 실기록·RPE, `GET /workout/{id}`) | ✅ 완료 |
...
| 확장 | 채팅 공지 고정 (pinned message) | ✅ 완료 |
| 확장 | 움직이는 이모티콘 30종 — PRO 게이팅·알림 미리보기(백엔드) + 패널·말풍선·미리보기(프론트) | ✅ 완료 |
| 확장 | 비개구리 이미지 스티커 10종 — 손그림 스케치를 이미지 모델로 스티커화한 자체 캐릭터, 번들 PNG(원가 0, 전부 무료), 프론트↔백엔드 동기화 테스트 | ✅ 완료(2026-09-08) |
| 요금제 | 우리 이모지 — 애인(또는 내) 사진 1장으로 감정 6종 캐릭터 이모지 세트를 AI 생성(V80, `AI_COUPLE_EMOJI` PRO 월 5세트, 커플 공용), 채팅 `COUPLE_EMOJI` 전송, 상대도 삭제 가능 | ✅ 완료(2026-09-08) — 실기기 검증·단가 확정 전 |
| 요금제 | 우리 이모지 무드 연동 — 내 얼굴 최신 세트 6장을 무드로 걸기(V81 `mood_statuses.couple_emoji_id`), 홈 배지 이미지 렌더, 이모지 삭제 시 유니코드 폴백 | ✅ 완료(2026-09-08) |
| 확장 | 채팅 맞춤법 제안 (기기 내 검사, 규칙+사전+오탐억제 3계층) + 설정에서 켜고 끄기 | ✅ 완료 |
| 확장 | 사진 저장·공유 공용 뷰어(`ImageViewer`) — 채팅·맛집·여행앨범·피드·식단기록에 적용 | ✅ 완료 |
| 확장 | 끼니 알림 — 아침/점심/저녁 등록 시간마다 그때까지 안 한 끼니만 리마인드, `NotificationCategory.REMINDER` 재사용 | ✅ 완료(2026-09-07) |
| 버그 수정 | 앱스토어 상품 페이지 언어 배지가 영어로 뜨던 문제 (`CFBundleDevelopmentRegion` 미지정) | ✅ 수정 완료(2026-09-07) |
| 커플 게임 | 미니게임 허브(홈 퀵액션 `게임`) — **협동 스도쿠**(둘이 같은 판을 차례 없이 채움, 틀린 칸 표시) + **오목**(15×15·5목, 판을 연 사람이 후공, 차례 검사, 2분 조용하면 "네 차례야" 푸시). 한 테이블 `couple_games`(V86·V87, 단일 테이블 상속, 행 잠금으로 동시 입력 보호), 끝나면 `GAME_CARD` 채팅 카드 + 푸시. 방향 결정·다음 후보(서로 맞히기·사진 직소·boo 오늘의 판)는 `docs/COUPLE_GAMES_DESIGN_2026-09-09.md` | ✅ 완료(2026-09-09) — 실기기 검증 전 |
...
**예외 하나 — `feed_reactions`(V60).** 반응 대상이 네 테이블(포스트·운동·식단·방문)로
넓어지면서 대상 쪽 FK 를 걸 수 없게 됐고, 그래서 `ON DELETE CASCADE` 도 잃었습니다.
**원본을 지우는 모든 경로가 반응도 함께 지워야 합니다** — `FeedService.deletePost` ·
`WorkoutService.delete` · `MealService.delete` · `PlaceService.deleteVisit`/`delete`,
그리고 두 purger. `user_id` 의 `users` FK 는 <b>일부러 남겨</b> 두었습니다: 정리에서
빠지면 탈퇴가 즉시 실패해 드러나기 때문입니다(안 그러면 조용히 유령 행만 쌓입니다).
`WithdrawFlowTest.운동_식단_맛집_카드에_반응을_남긴_계정도_탈퇴할_수_있다` 가 이걸 지킵니다.
...
**Dubly 는 사진 창고가 아닙니다.** 갤러리 전체 자동 백업·공유는 구글포토에 맡기고,
Dubly 는 *맥락이 붙은 기록*(어느 여행 며칠차, 무엇을 먹고 어디를 갔는지, 스트릭·반응)만 담습니다.
