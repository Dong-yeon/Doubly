 ...
 * 뒤져야 하면 반드시 어긋난다.
 *
 * <p><b>⚠️ 아래 FREE 숫자는 2026-09-22 부터 실제로 강제된다.</b> 그전까지는 전역 무료
 * 체험({@code fitto.plan.free-trial=true})이라 한 줄도 실행되지 않는 자리표시자였다.
 * 체험을 끄면서 <b>숫자는 그대로 둔 채</b> 켰다 — 실사용 분포(p60~p75)를 재려면 먼저
 * 무료 사용자가 존재해야 하고, 지금은 그 상태를 만드는 단계다. 처음 막히는 자리가
 * 나오면 여기 숫자부터 의심할 것. 고칠 곳은 이 파일 하나다.
 *
...
public enum Feature {
     ...
     * 키로만 돌고, <b>장당</b> 약 0.04 USD 가 나간다 — 세트는 감정 전체를 그리는 것이라 감정을 늘리면
     * 세트 원가도 같이 오른다(감정 17종 기준 세트당 약 0.68 USD). 그래서 무료는 맛보기 없이 막고,
     * PRO 도 월 상한을 둔다.
     *
     * <p><b>월 5 → 3 (2026-09-10).</b> 감정이 6종에서 17종이 되면서 세트 원가가 0.23 → 0.68 USD 로
     * 올랐다. 5세트를 그대로 두면 사용자당 월 원가가 약 3.4 USD 로, PRO 가격 대비 감당할 수 없다.
     * 3세트면 약 2.0 USD 다. 부분 재생성(마음에 드는 장은 두고 나머지만 다시)이 들어와서 실제 소모는
     * 이보다 낮다 — 한 번에 전체를 다시 뽑을 이유가 줄었다.
     *
     * <p><b>월 3 → 4 (2026-09-11).</b> 한 요청이 5장으로 제한되면서
     * ({@code CoupleEmojiService.MAX_EMOTIONS_PER_REQUEST}) "1회 = 한 세트 전체"가 아니게 됐다.
     * 17종을 갖추려면 4회(5+5+5+2)가 필요한데 3회로는 <b>한 달 안에 한 벌도 완성할 수 없다</b> —
     * 한도의 목적은 원가 방어지 완성 자체를 막는 것이 아니다.
     *
     * <p>원가는 오히려 내려간다. 1회가 최대 5장이므로 월 상한은 4 x 5 = 20장 = 약 0.8 USD 다
     * (이전 3 x 17 = 51장 = 약 2.0 USD). 상한을 올리면서 원가가 60% 줄어드는 건, 예전 "1회"가
     * 17장짜리 덩어리여서 한 번만 써도 비쌌기 때문이다.
     * docs/COUPLE_EMOJI_AI_DESIGN_2026-09-08.md §6·§8.
     */
    AI_COUPLE_EMOJI("우리 이모지 만들기", Quota.blocked(), Quota.perMonth(4)),
     ...
     *
     * <p><b>한도는 커플이 함께 쓴다</b>({@link #isCoupleScoped()} · {@code PlanGuard.scopeOf}).
     * 사진은 대부분 커플 공간에 쌓이고, 둘 중 한 명이 주로 찍는 게 보통이다 — 사람마다 따로
     * 세면 같은 앨범을 보면서 한쪽만 먼저 막힌다.
     *
     * <p>그래서 무료 한도가 <b>30 → 60</b> 이 됐다. 숫자를 늘린 게 아니라 <b>총량을 보존</b>한
     * 것이다: 예전엔 각자 30장(커플 합 60)이었고 지금은 둘이 합쳐 60장이다. 혼자 쓰는 동안에는
     * 커플이 없어 본인 주머니로 떨어지므로, 연결 전 사용자에게는 실질 상향이다(그만큼이 이
     * 기능의 체험이 된다).
     *
     * <p>PRO 의 1000장은 판매 문구가 아니라 <b>사고 방지선</b>이다. 앱 버그나 자동화로
     * 무한 업로드가 돌면 요금이 그대로 청구되므로, "무제한"이라도 절대 상한은 둔다. 이 값은
     * 커플 단위로 두는 편이 원래 의도에 더 맞는다 — 요금 폭탄 방지는 계정 수와 무관하고,
     * 커플 월 1000장이면 실사용에서는 닿지 않는다.
     */
    PHOTO_UPLOAD("사진 업로드", Quota.perMonth(60), Quota.perMonth(1000)),
