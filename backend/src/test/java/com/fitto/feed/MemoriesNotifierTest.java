...
class MemoriesNotifierTest {
    ...
    void 커플_양쪽에_같은_문구로_보낸다() {
        ...
        verify(notificationService).notify(eq(c[0]), eq(NotificationCategory.REMINDER), eq("우리 추억"), eq(body), anyString());
        verify(notificationService).notify(eq(c[1]), eq(NotificationCategory.REMINDER), eq("우리 추억"), eq(body), anyString());
    }
    ...
    void 끊긴_관계에는_보내지_않는다() {
        ...
        verify(notificationService, never()).notify(eq(c[0]), any(), eq("우리 추억"), any(), any());
        verify(notificationService, never()).notify(eq(c[1]), any(), eq("우리 추억"), any(), any());
    }
}
