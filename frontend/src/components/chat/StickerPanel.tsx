 ...
 *
 * <p><b>320px 로 올렸다(2026-09-22).</b> 240px 이던 근거는 "가장 큰 팩이 14장이라 3줄이면
 * 다 보인다 — 더 높이면 아래가 빈다"였다. 같은 날 8팩을 4팩으로 합치면서(V100) 한 팩이
 * 18~34장이 됐으므로 <b>그 근거가 사라졌다</b>. 이제는 높이가 남는 게 아니라 모자란다.
 *
 * <p>320px 의 셈: 스트립 40 + 여백 16 을 빼면 264px 이고, {@link CELL_SIZE}(46) + 간격의
 * 한 줄이 약 52px 이라 <b>5줄</b>이 들어간다. 줄당 개수는 폭이 정하므로(CELL_SIZE 주석)
 * 폰이면 30장쯤, 태블릿이면 그보다 훨씬 많다 — 예전 고정 6칸 3줄(18장)과 대비된다.
 *
 * <p>상한일 뿐이라 키보드가 그보다 크면 상한을 따른다({@code panelHeight}).
