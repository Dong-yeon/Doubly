...
export function WorkoutCalendarScreen({ navigation }: Props) {
  const now = new Date();
  ...
  // 실패해도 빈 Set 으로 조용히 넘어가면 "이번 달 운동 안 했나?"로 오해한다
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      {/*
        날짜를 누를 수 있게 한다 — 예전엔 View 라 눌러도 반응이 없어서,
      ...
      {selectedDay ? (
        <View style={styles.selectedBox}>
          <Text style={styles.selectedText}>
            ...
            {completedDays.has(selectedDay) ? '운동한 날이에요' : '기록이 없어요'}
          </Text>
          <Button
            title="이 날 기록하기"
            size="md"
            variant="secondary"
            onPress={() =>
              navigation.navigate('WorkoutRecord', { date: dateStringOf(year, month, selectedDay) })
            }
          />
        </View>
      ) : (
...
const styles = themedStyles((colors) => ({
  ...
  /* 선택 표시는 테두리로 — 완료(채움)와 겹쳐도 둘 다 읽힌다 */
