...
> **상태: 방향 결정 + 구도 실측(§11, 2026-09-23). 에셋·코드 변경 없음.** 저장소의 아이콘 5종은 아직 §10 시점
...
| **"둘"이 구조에 내장** | 쌍란 = 노른자 2개. ~~`colors.ts` 의 `나 = Gold` / `상대 = Green` 과 짝이 맞는다~~ — **색 근거는 2026-09-23 에 뺐다(§11-2)**. 구도상 "둘"이라는 것만 남는다 |
...
| 추상 | **Q5** 하트 하나를 2색(Gold/Green)으로 가르되 **이음매를 S자로** | ~~현 시점 최선~~ → **폐기(2026-09-23).** 캐릭터가 정해져 이 계열 자체를 쓰지 않는다. 다만 "이음매는 S 자" 판정은 살아 있다 |
...
그 문제가 없다. 곡선은 `DoublyLogo.tsx` 의 `heartPoints` 그대로라 인앱 심볼과 같은
하트를 쓴다.

> ~~Gold/Green 듀오톤이 핑크 중앙값에서 벗어나 있고 `colors.ts` 의 `나 / 상대` 의미축을
> 그림으로 만든다~~ — **이 근거는 2026-09-23 에 뺐다(§11-2).** 살아 있는 것은 "하트를
> 내린 이유는 아웃라인·그라데이션이었지 하트여서가 아니다"와 "이음매는 S 자" 둘뿐이다.
...
| 계란 두 알 구도 | **실측 완료 — §11.** 원래 메모: §8(선행)이 실측한 함정 있음 — **48px 에서 눈 4개가 일렬로 붙어 "눈 넷 달린 한 마리"로 읽힌다.** 해법(마주 보는 옆모습으로 눈을 2개로)을 계란에 적용해봐야 함 |
...
## 11. 계란 두 알 구도 — 실측 (2026-09-23)

§8 이 "미탐색"으로 남긴 항목이다. 그 사이 **계란 캐릭터가 실제로 앱에 들어왔다** —
달걀이 23종 · 구운이♥달걀이 20종(V99, 이름은 V103·V104). §3 이 적은 "아이콘이 앱에
존재하지 않는 캐릭터를 쓰고 있다"가 닫힐 수 있는 첫 시점이다. 이름은 **달걀이(흰) ·
구운이(갈색)** 로 확정했다(2026-09-23).

후보는 **실제 스티커 아트를 조립해** 만들었다(새로 그리지 않았다 — 지금 알고 싶은 것은
그림 품질이 아니라 **축소 생존**이다). 떠 있는 장식(깃털·하트·zzz)은 연결 성분 크기로
떼어냈다. 흰 배경 1024², 어댑티브 원형 마스크를 감안해 마크를 중앙 76% 안에 가뒀다.

| 후보 | 마크 점유율 | 평균 ΔL\* | 24px 판독 | 판정 |
| --- | --- | --- | --- | --- |
| A 구운이 단독 | 29.1% | 44.8 | 31.6% | **24px 에서 형태가 온전한 유일한 캐릭터 안.** "둘"이 없다 |
| B 나란히 | 31.1% | 37.5 | 31.8% | 48px 부터 흰 달걀이가 배경에 녹고 **꽃만 노란 점**으로 남는다 |
| C 마주보기 | 30.1% | 36.5 | 30.6% | B 와 같은 문제 + 24px 에서 둘이 한 덩어리로 붙는다 |
| **D 갈색 앞·흰 뒤** | **36.4%** | 39.3 | 37.3% | **두 마리 안 중 최선.** 갈색이 실루엣을 지고 꽃이 포인트로 남는다 |
| E 쌍란 추상(얼굴 없음) | 15.6% | **66.2** | 24.3% | 대비·판독 최고. 그러나 캐릭터가 아니고 **앱에 쌍란이 없다**(§3 문제 재발) |

**§5 주의1 이 실측으로 확인됐다.** 흰 달걀이는 마크 점유율에 거의 기여하지 못한다 —
B 가 A 보다 점유율이 2%p 높을 뿐인데, 흰 몸이 배경과 갈리지 않아 **검은 윤곽선만** 잡히기
때문이다. 두 마리를 쓰려면 **갈색이 앞에서 실루엣을 져야 한다**(D).

**F(구운이 + 달걀이의 꽃)는 렌더하지 못했다** — 꽃이 달걀이 윤곽선과 한 덩어리라 기존
아트에서 떼어낼 수 없다. 이 구도로 가려면 **꽃을 새로 그려야 한다.**

### 다음 단계에 넘기는 제약 (그림을 새로 그릴 때)

1. 흰 배경 유지 → **채운 형태**, 얇은 아웃라인 금지(§4)
2. **갈색 구운이가 잉크 질량**을 진다. 흰 달걀이는 보조
3. 눈은 **2개로 읽혀야** 한다(§8 의 "눈 넷" 함정) — 감은 눈·옆모습이 해법
4. 볼터치·혀·떠 있는 왕관/하트/깃털 **전부 제거**(§4)
5. **구데타마 회피**(§5 주의2): 차분하되 늘어지지 않게. 세 라운드가 "장난 같다"로 실패했다
6. 중앙 76% 안전 영역, 1024² + 어댑티브 432² + 스토어 512²
7. 스티커 원본은 360×305 라 **확대해서 쓸 수 없다**

재현: 렌더 스크립트는 저장소에 넣지 않았다(§10 관례). 산출물은 `.icon-candidates/`
(gitignore).

### 11-1. D 안 실물 평가 — 그리고 §11 예측 하나가 틀렸다 (2026-09-23)

D 구도로 이미지 생성(나노바나나)한 첫 실물을 같은 축으로 쟀다.

| | 마크 점유율 | 평균 ΔL\* | 좌(갈색) ΔL\* | 우(크림) ΔL\* | 24px 잉크 좌:우 |
| --- | --- | --- | --- | --- | --- |
| D 목업(스티커 조립) | 36.4% | 39.3 | — | — | — |
| **D 실물** | 35.3% | **46.6** | 53.0 | 38.9 | 120 : 86 |

**§11 의 "흰 달걀이는 축소에서 사라진다"가 실물에서는 성립하지 않았다.** 48px 에서도 둘 다
읽힌다. 원인은 선이다 — 스티커 아트의 **가늘고 떨리는 손그림 선** 대신 **굵고 균일한 벡터
외곽선**을 쓰면 크림 몸도 자기 면적을 지킨다. §5 주의1("흰 껍질로 그리면 안 된다")은
*채움색*에 대한 경고였고, **외곽선 굵기라는 변수를 빠뜨리고 있었다.**

**고칠 것 하나 — 어댑티브 아이콘에서 발이 잘린다.** 마크가 폭 71% 라 어댑티브 안전원
(중앙 66%) 밖으로 **마크 화소의 10.2%** 가 나간다. `icon.png`(1024²·iOS·스토어)는 그대로 써도
되지만 `android-icon-foreground.png` 는 **마크를 66% 로 줄인 별도 파일**이어야 한다.
같은 파일을 쓰면 런처 마스크에서 발과 꽃 오른쪽이 깎인다.

### 11-2. "달걀로 하트" — 배치로는 안 되고, 반쪽을 크림으로 두면 깨진 하트가 된다

`DoublyMark`(인앱 로고)가 이미 하트라, 두 알이 하트를 이루면 §7 의 미정("아이콘도 추상,
인앱도 추상인데 둘이 다른 하트")이 닫힌다. 개념은 강하다. 다만 둘을 쟀다.

1. **기존 아트를 기울여서는 하트가 안 나온다.** 달걀은 위가 좁고 아래가 넓은데 하트는 그
   반대다(위 두 로브·아래 한 점). 회전·배치로 접근한 프로토타입은 전부 실패했다 —
   **실루엣을 처음부터 하트로 그려야 한다.**
2. **반쪽을 크림으로 두면 좌우가 2배 가까이 기운다.** 하트를 갈색/크림으로 가른 시안은
   좌우 평균 ΔL\* 차이 **31.9**(D 실물은 14.1), 24px 잉크 **171 : 89**. 이건 §6 이 Q1(직선
   이음매)을 "깨진 하트로 읽힐 위험 — 커플 앱에서 치명적"이라며 뺀 그 실패와 같은 그림이다.

**해법 두 갈래** — ① 달걀이 쪽을 크림 대신 더 진한 색으로 채운다(예: 꽃의 골드).
근거는 <b>대비 하나뿐</b>이다 — 위 두 수치를 좁히는 것. ② 하트 실루엣은 **갈색 하나로**
두고 흰 알을 그 앞에 겹친다(D 의 잉크 배분을 하트에 적용).
어느 쪽이든 이음매는 **직선이 아니라 S 자**여야 한다(§6 Q1 판정).

> **Gold/Green 의미축은 근거에서 뺀다 (2026-09-23).** §5 의 "둘이 구조에 내장"과 §6 의 Q5
> 판정이 `colors.ts` 의 `나 = Gold` / `상대 = Green` 을 색 선택의 근거로 삼았는데, 그건
> **캐릭터가 없던 시절의 추상안 논리**다. 지금은 달걀이·구운이가 자기 팔레트(갈색 + 크림 +
> 꽃 골드)를 갖고 있으므로 색은 그 팔레트와 대비로 정한다. <b>이 축을 다시 끌어오지 말 것.</b>
