...
1. **그림 보낼 때** 채팅방에 그림이 **사진으로** 남고, `🎨 캐치마인드예요! 시작해보세요`
   한 줄이 붙는다(§4-1).
2. **상대가 틀릴 때마다** 그 시도가 채팅 카드로 남는다(앞 5번까지).
3. 부수적으로, 같은 세션에서 발견한 버그 **넷**을 같이 고쳤다(아래 §5).
...
사진만 덜렁 올라가면 받는 쪽은 이게 게임인지 그냥 사진인지 모른다. 푸시 알림
("이게 뭘까? 🎨")은 **알림을 끈 사람에게 닿지 않으므로** 채팅 자체에 한 줄이 있어야 한다.

`IMAGE` 말풍선이 `content` 를 무시하고 사진만 그리고 있었으므로(`ChatRoomScreen.tsx` 의
`isImage` 분기) 렌더도 함께 고쳤다.

- 말풍선 없이 배경 위에 놓으므로 색은 `chat.meta` — **배경 대비 4.5:1 이 20개 팔레트 전부
  검증돼 있다**(`verify-chat-theme-contrast.mjs` 의 '배경 위 시간·날짜' 규칙). 새 색을
  만들지 않은 이유가 이것이다.
- 캡션은 `Pressable` **밖**에 둔다 — 캡션 탭이 전체화면을 여는 건 기대와 다르다.
- 사용자가 보내는 사진에는 본문이 없으므로(`ChatService.send`) **기존 사진 메시지의 모양은
  그대로다.** 조건부 렌더라 구버전 앱에서도 사진만 보이고 깨지지 않는다.
- 인용 미리보기(`ChatService.preview`)도 `content` 를 보게 했다. `"[이미지]"` 로 나오면
  본문이 멀쩡히 있는 메시지가 빈 것처럼 보인다.

제시어는 넣지 않는다 — §4 와 같은 이유다. 테스트로 고정했다
(`.contains("캐치마인드").doesNotContain("우산")`).
...
### 5-4. 오른쪽으로 긋기만 하면 화면이 닫혔다 (`0f5263e`)

`headerOptions.tsx:70-71` 이 **전역으로** `gestureEnabled: true` + `fullScreenGestureEnabled: true`
를 켜 둔다. 후자는 왼쪽 가장자리가 아니라 **화면 어디서든** 오른쪽으로 긋기만 하면 pop 이다.
이 화면의 절반이 캔버스라, 오른쪽으로 긋는 획마다 화면이 닫히고 그 순간 미전송 그림이
사라졌다 — §5-2 와 같은 유실이 다른 경로로 하나 더 있었던 것이다.

`ChatStackNavigator` 의 `ChatRoom` 이 이미 같은 조치를 해 뒀고(2026-09-01·09-02 실기기
리포트, 그쪽 주석에 배경이 다 있다) 같은 방식으로 둘 다 껐다.

두 가지는 일부러 하지 않았다.

- **"그리는 중에만" 끄기** — §5-1 의 스크롤 잠금은 경쟁에서 져도 최악이 몇 px 흔들림인데,
  이건 지면 **화면 이탈 + 그림 소실**이다. 위험의 크기가 달라 정적으로 끈다.
- **`SwipeBackView` 붙이기** — 화면 전체에서 오른쪽 스와이프를 받는 커스텀 제스처라
  방금 끈 문제를 JS 로 다시 만드는 셈이다. 뒤로가기는 커스텀 `headerLeft` 버튼과
  안드로이드 시스템 백으로 충분하다.
...
| 항목 | 내용 |
|---|---|
| **실기기 검증** | 이 문서의 모든 항목이 계산·테스트 기반이다. `toDataURL` 실제 렌더, 투명 배경 여부, iOS 스크롤 잠금 타이밍, 되살리기 동작, 캡션 가독성(특히 다크) 전부 **미검증** |
| 가로 스크롤 vs 풀스크린 제스처 | §5-4 의 `fullScreenGestureEnabled` 는 **전역**이라 가로로 긋는 다른 자리와도 경쟁한다 — `PlaceScreen` 카테고리 칩 행, `StickerPanel` 스트립, 채팅 이미지 미리보기 스트립. 보고된 증상이 아니고 실기기 확인도 없어 손대지 않았다. "칩을 오른쪽으로 밀면 화면이 닫힌다"가 나오면 이 문제다 |
| 공유 실패의 구분 | `shareToChat` 이 렌더·서명·업로드 실패를 **전부 삼킨다**(의도). 그래서 "채팅에 사진이 안 옴" 하나로 보이고 원인을 구분할 수 없다. 필요하면 구분용 로그를 넣는다(JS 만이라 OTA) |
