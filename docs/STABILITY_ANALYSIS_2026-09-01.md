...
> 4번(202 + 폴링)은 여행 일정·주간 레터에 적용 — 아래 "C-4 후기" 참고.
...
**남은 것**: 나머지 AI 기능(음식 사진·식단 코치·데이트 코스·맛집 추천)은 아직 동기다.
같은 기계를 쓰면 각각 몇 줄이면 되지만, 상대적으로 짧은 경로라 이번 범위에서 뺐다.
초기 요청자에게 완료 푸시를 보내는 것도 안 했다 — 파트너 푸시는 원래대로 나간다.
