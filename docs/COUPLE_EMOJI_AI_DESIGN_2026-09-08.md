...
## 20. 비개구리 2차 — 8종 추가, 세트 18종 (2026-09-14)
...
새 것을 뒤에 붙이지 않고 18종을 감정 흐름대로 재배열했다(기쁨 → 애정 → 신남 → 삐짐·화 → 슬픔 →
...
룰루랄라는 음표가 있다. 다만 `BIGAE_DANCE`(뒷모습+음표)와 1차 `BIGAE_WIGGLE`(뒷모습 엉덩이)은
실루엣이 같아 음표 하나로만 갈린다 — 사용자 판단 대기.

### 남은 것

- 상호작용 스티커(꽃단장·안기·이뻐해주기) — 두 마리 구도 스케치 대기(§17 그대로).
- 실기기에서 트레이 확인. 이제 이미지 스티커가 28장이라 "이모티콘" 탭 스크롤이 길어졌다.
