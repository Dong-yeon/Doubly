...
이어서 비개구리 2차 스케치 8종을 붙였다 — §20.

## 20. 비개구리 2차 — 8종 추가·1종 제거, 세트 17종 (2026-09-14)

사용자가 새 스케치 한 장(흰 줄노트, 한 페이지에 그림 11개)을 올렸다. §15 와 같은 경로로 태웠다:
`crop_bigae2.py` → `sketch.mjs`(참조 1장) → `normalize_stickers.py`.

| 코드 | 라벨 | 스케치 |
|---|---|---|
| `BIGAE_LIKE` | 좋아 | `><` 눈 + 세모 입 |
| `BIGAE_HEHE` | 히히 | 큰 웃음 + 혀 |
| `BIGAE_KISS` | 뽀뽀 | 하트 눈 + `ε` 입 |
| `BIGAE_GIFT` | 선물이야 | 리본 상자를 안은 전신 |
| `BIGAE_DANCE` | 룰루랄라 | 뒷모습 + 음표 |
| `BIGAE_GRUMPY` | 짜증나 | 치켜뜬 눈 + 짜증 표시 |
| `BIGAE_DIZZY` | 어질~ | 소용돌이 눈 + 혀 |
| `BIGAE_OFFWORK` | 퇴근 | 지하철 손잡이 + 회사가방 |

**뺀 것 3개**: 가장자리에 걸려 잘린 그림 2개, 그리고 낙서로 지워진 "지하철 손잡이" 초안
(같은 구도의 깨끗한 판이 `08-offwork` 에 있다).

### 1차와 달랐던 것 — 종이가 희고 윤곽이 연필이다

1차는 분홍 종이에 볼펜이라 선이 저절로 떴다. 이 페이지는 **흰 줄노트에 얼굴 윤곽은 연필,
이목구비는 볼펜**이다. 자른 그대로 넘기면 모델이 하트형 머리를 못 보고 **눈·입만 공중에 떠 있는
그림**을 그린다. `crop_bigae2.py` 가 자른 뒤 `autocontrast(cutoff=(0.5, 12))` 로 종이를 흰색까지
민다 — 자르기 **전**에 걸면 페이지 전체 밝기 기준이라 각 그림에서는 거의 효과가 없다.

크롭 좌표도 원본 픽셀(1440 기준)로 적고 다른 해상도면 비율로 환산한다. 1차(`crop_frog.py`)는
692 기준 눈대중 좌표를 실제 크기에 맞춰 늘리는 방식이라, 사진 해상도가 바뀌면 조용히 어긋났다.

### `--label` 은 쉼표로 나뉜다 (돈 나간 실수)

`sketch.mjs --label` 은 `split(',')` 이다. 8종을 `|` 로 이어 넘겼더니 **첫 장이 이어붙인 문자열
전체를 감정 힌트로 받고 나머지 7장은 힌트 없이** 돌았다. 8장 전부 "OK" 로 찍혀서 로그만 봐서는
모른다. 결과를 눈으로 보고 나서야 알았다 — 선물/뒷모습/퇴근처럼 **소품과 구도가 핵심인 그림**이
전부 틀렸다(선물 → 포장지에 감긴 캐릭터, 뒷모습 → 앞모습, 지하철 손잡이 → 풍선). 표정만 있는
그림 3장은 힌트 없이도 맞았다. 라벨을 제대로 넣어 5장을 다시 뽑아 해결했다.

**교훈**: 감정 힌트는 "없어도 되는 보조"가 아니다. 표정뿐인 그림에서는 그렇지만, 소품·구도가
있는 그림에서는 그것이 유일한 단서다. 그리고 이 스크립트의 라벨에는 쉼표를 쓸 수 없다.

선물 한 장은 세 번 뽑았다 — 힌트를 넣어도 포장 색이 크림·하늘색으로 빠져 세트의 연두 팔레트에서
떴다. 상자 색을 프롬프트에 박아(`soft pink paper with a red ribbon`) 잡았다. 총 14장 생성.

### 순서를 다시 늘어놨다

새 것을 뒤에 붙이지 않고 전체를 감정 흐름대로 재배열했다(기쁨 → 애정 → 신남 → 삐짐·화 → 슬픔 →
지침 → 잘자). 뒤에 붙이면 "좋아"가 17번째에 앉아 스크롤을 내려야 나온다. **이 세트는 아직 실사용
이력이 없어 자리를 외운 사람이 없다** — 지금이 아니면 못 고친다. 코드는 그대로라 과거 말풍선은
영향받지 않는다(순서는 프론트 카탈로그에만 있다 — §19).

### 라벨이 기존과 부딪히는 곳 3개

사용자가 그림의 뜻을 "화남 / 좋아 / 신나~"로 알려줬는데 셋 다 1차에 같은 이름이 이미 있다
(`BIGAE_ANGRY` 화났어 · `BIGAE_LOVE` 좋아좋아 · `BIGAE_EXCITED` 신났어). 트레이에 "화났어"와
"화남"이 나란히 서면 무엇이 다른지 읽히지 않으므로 **짜증나 · 좋아 · 룰루랄라**로 비틀어 넣었다.
그림 자체는 겹치지 않는다 — 화남은 반쯤 감은 째림(기존은 눈 부릅뜸), 좋아는 `><`(기존은 혀+하트),
룰루랄라는 음표가 있다.

`BIGAE_DANCE`(뒷모습+음표)와 1차 `BIGAE_WIGGLE`(뒷모습 엉덩이)은 실루엣이 같아 음표 하나로만
갈렸다. **사용자 판단으로 `BIGAE_WIGGLE` 을 뺐다** — 뜻이 분명한 쪽(음표가 있는 룰루랄라)을 남겼다.
그래서 비개구리는 10 + 8 − 1 = **17종**이다.

없는 코드를 만나면 말풍선은 `stickerImageOf` 가 undefined 를 돌려주어 **코드 문자열이 그대로
글자로 뜨고**(`ChatRoomScreen` 1248행), 푸시 미리보기도 `StickerImage.from` 이 empty 라 라벨을 못
붙인다. 배포 전이라 `BIGAE_WIGGLE` 로 보낸 메시지가 실제로 없어서 지울 수 있었다 — **이미 나간
코드는 이렇게 못 지운다.** 지워야 하면 트레이에서만 감추고 enum·에셋은 남겨야 한다.

### 남은 것

- 상호작용 스티커(꽃단장·안기·이뻐해주기) — 두 마리 구도 스케치 대기(§17 그대로).
- 실기기에서 트레이 확인. 이제 이미지 스티커가 27장이라 "이모티콘" 탭 스크롤이 길어졌다.

## 21. 비개구리 3차 — 몸통을 모찌로 갈고 남녀 두 마리로 (2026-09-15)

사용자가 3차 스케치(한 페이지 7종)를 올린 뒤 방향이 두 번 바뀌었다: ① 기존 비개구리 몸통을
**모찌(슬라임) 형태**로 전면 교체 ② 같은 그림을 **초록(남)·분홍(여) 두 캐릭터**로 분리.
결과는 이미지 스티커 38장 — 비개구리(남) 14 · 비개구리(여) 14 · 곰돌이 10.

### 가장 비싼 교훈 — 모양을 글로 설명하면 매번 새로 그려진다

몸통 후보를 뽑는 동안 같은 실수를 반복했다. `sketch.mjs` 는 호출마다 이미지를 **처음부터 다시
그린다.** 모양을 프롬프트 문장으로만 쥐고 있으면 호출마다 실루엣이 새로 만들어진다. 그래서
"색만 진하게 해달라"는 요청에도 모양이 같이 흔들렸고("색을 넣었는데 왜 모양도 같이 변해"),
하트 골이 사라지고, 바닥에 없던 턱이 생기고, 이마가 넓어지는 판을 차례로 버렸다.

**순서를 뒤집고서야 풀렸다.**

1. 기준 원본 한 장을 먼저 굳힌다 → `scripts/couple-emoji-experiment/bigae-anchor.png`
2. 그 그림을 `--ref` 로 박아서 나머지를 뽑는다 (`restyle.mjs`)
3. **색은 생성이 아니라 후처리다** (`unify_body_color.py`)
4. 미세 조정도 후처리다 — "이마가 넓다"는 얼굴 픽셀만 40px 위로 옮겨 해결했다. 다시 그리지 않았다

특히 3번은 수치로 드러났다. 프롬프트로 색을 지정해 뽑은 24장의 몸통 중앙값이 `#71BA68` 부터
`#99D490` 까지 벌어져 있었다. 후처리로 `#86CB7F`(초록) / `#E58FA9`(분홍) 한 색이 됐다.

### 원래 스케치가 아니라 완성 스티커를 입력으로 썼다

`restyle.mjs` 는 **완성본 두 장**(기준 원본 + 기존 스티커)을 주고 "앞은 몸, 뒤는 표정"이라고
못박는다. 스케치로 되돌아가지 않은 이유는 두 가지다 — 1차 10종은 **어느 스케치가 어느 코드가
됐는지 매핑이 남아 있지 않았고**(손으로 골라 넣었다), 스케치에서 다시 뽑으면 표정·소품이 또
흔들린다. 완성본을 주면 바뀌는 건 몸통뿐이다.

### 팔을 없앴더니 들고 있던 물건이 떴다

기준 원본은 다리가 없고 뭉툭한 손만 살짝 나온다. 그랬더니 물건을 **들고 있던** 장에서 손잡이·
가방·편지·상자·거울이 공중에 떴다. 그 몇 장만 `--extra` 로 "팔을 늘려서 실제로 쥐게 하라"를
덧붙여 따로 돌렸다. 전량에 붙이면 나머지 장까지 팔이 자라난다.

이 외에 따로 손본 것: 앵커를 무시하고 하트 몸으로 돌아간 장(뽀뽀), 없던 소품이 생긴 장
(꽃단장에 선물상자), **IMAGE 1 의 얼굴을 베껴 와 다른 장과 같아진 장**(신났어가 좋아와 같아짐 —
"7번은 없어도 될 것 같다"는 판단이 나올 뻔했는데 실제로는 내 파이프라인 결함이었다).

### 색 후처리에서 얼굴이 지워진 건

초록 계열 픽셀을 전부 목표색으로 누르니, **이목구비를 짙은 올리브로 그려 둔 장**(화났어)의
눈·눈썹이 몸통색에 먹혀 통째로 사라졌다. 명도 0.55 미만은 "짙은 선"으로 보고 색상만 돌린다.
가장자리 안티앨리어싱 픽셀(채도가 낮다)도 같은 이유로 색상만 돌린다 — 평면화하면 테두리가
계단처럼 깨진다.

### 구성

공용 4종(윙크·시무룩·짜증나·축 처짐)은 양쪽에 같은 그림이 색만 바뀌어 들어간다. 둘 다 쓰는
표정이라 한쪽에만 두면 나머지 한 사람이 못 쓴다.

| | 비개구리(남) 초록 | 비개구리(여) 분홍 |
|---|---|---|
| 전용 10 | 좋아 · 히히 · 하하하 · 신났어 · 룰루랄라 · 선물이야 · 화났어 · 흥 간다 · 어질~ · 퇴근 | 좋아좋아 · 뽀뽀 · 방긋 · 흐뭇 · 기분 좋아 · 꽃단장 · 예뻐졌지? · 어머 · 엉엉 · 잘자 |
| 공용 4 | 윙크 · 시무룩 · 짜증나 · 축 처짐 | 같은 4종 (`BIGAEF_` 접두) |

`BIGAE_LOVE`·`KISS`·`CRYING`·`SLEEPY` 는 분홍 쪽으로 옮겨 `BIGAEF_` 가 됐다. 배포 전이라 그
코드로 보낸 메시지가 없어서 가능했다 — **이미 나간 코드는 이렇게 못 옮긴다**(§20).

### 아직 아쉬운 것

- **분홍 판의 볼 홍조**가 분홍 몸통 위에서 베이지처럼 보인다. 홍조만 따로 더 진한 장미색으로
  옮기면 되지만 후처리 규칙을 하나 더 얹어야 한다.
- 앱 아이콘(`bigae-mark.svg`·`BigaeMark`)은 **일부러 안 바꿨다** — 아이콘은 fingerprint 입력이라
  바꾸면 OTA 가 아니라 새 빌드 + 스토어 재제출이 된다(`docs/OTA_PREFLIGHT_2026-09-14.md`).
  그래서 지금 아이콘은 뾰족한 하트인데 채팅 안 캐릭터는 모찌다. 다음 빌드 때 같이 맞출 것.
- 실기기 확인 전무. 트레이 구획이 셋이 되어 "이모티콘" 탭 스크롤이 더 길어졌다.
- 생성 비용: 이 세션에서 이미지 약 66장. 중간에 선불 잔액이 한 번 소진됐다.


## 22. 분홍 대신 노랑, 이름은 더비·블리 (2026-09-15)

§21 의 분홍(여) 판을 **노랑**으로 바꾸고, 두 캐릭터에 이름을 줬다 — **더비(초록) · 블리(노랑)**.
앱 이름 "더블리"를 둘로 쪼갠 것이고, 트레이 구획도 그 이름으로 읽힌다.

**분홍을 버린 이유**는 "너무 여자여자하다"였다. 성별로 가른 구성(꽃단장·예뻐졌지?가 한쪽에)
자체는 그대로 두되 색에서 성별 신호를 뺐다. 노랑이 개구리라는 종에도 더 맞는다.

**"비개 / 구리"로 쪼개는 안은 걸렀다** — "구리"가 구리다로 읽힌다.

### 볼 홍조 — 생성물의 홍조는 원래 탁한 살구색이었다

분홍 몸통 위에서 홍조가 베이지로 묻힌다는 문제(§21 "아직 아쉬운 것")를 뜯어보니, 생성물의
홍조가 애초에 `#D3A18A` 같은 **탁한 살구색**이었다. 초록 위에서는 대비가 있어 분홍으로 보였을
뿐이다. `unify_body_color.py` 에 홍조 목표색을 넣어 블리만 산호색(`#E8896F`)으로 누른다.
더비는 원래 대비가 충분해서 건드리지 않는다(`BLUSH_TARGETS['green'] = None`).

홍조를 잡는 규칙은 몸통과 같은 함정을 갖는다 — **명도 0.55 미만은 빼야 한다.** 안 빼면 갈색
윤곽선이 산호색으로 물든다.

### 색상만 돌리는 방식의 한계 — 소품 두 장은 다시 뽑았다

초록이던 **소품도 같이 노래진다**. 리본이 몸통과 같은 색이 되고 `좋아좋아`의 하트도 노란
하트가 됐다. **후처리로는 되돌릴 수 없다** — 원본(초록)에서도 소품이 몸통과 같은 초록이라
색으로 가를 단서가 없다. 그래서 그 두 장만 다시 뽑았다.

방법: `bigae-anchor.png` 를 노랑으로 돌려 `bli-anchor.png` 를 만들고, 그걸 `--ref` 로 박은 뒤
`--extra` 로 소품 색만 지정했다(soft rose pink). **색 후처리는 건너뛴다** — 돌리면 장밋빛 소품이
홍조 범위에 걸려 산호색으로 끌려간다. 노란 앵커로 뽑았더니 몸통이 `#F1CC62` 로 나와 세트 기준
`#F2CF6A` 와 눈에 안 보이는 차이라 그대로 썼다.

`좋아좋아`는 두 번 뽑았다. 첫 판은 하트는 분홍이 됐는데 **팔이 몸통과 같은 노란색으로 그려져
사라졌다.** "팔은 얼굴선과 같은 진한 갈색 윤곽으로 그리고 몸통 색으로 그리지 말 것"을 박아
해결했다 — 색을 지정할 때는 **그리지 말아야 할 색도** 같이 적어야 한다.

### 코드 이름도 바꿨다

`BIGAE_*` → `DUBI_*`, `BIGAEF_*` → `BLI_*`. `BIGAEF_` 의 F 는 female 이었는데 성별 프레임을
버렸으므로 뜻이 어긋난 채 남는다. 배포 전이라 가능했다 — **이미 나간 코드는 못 바꾼다**(§20·§21).
에셋 파일도 `dubi_*` · `bli_*` 로 맞췄다.

검증: npm run typecheck, StickerImageSyncTest, build:web(dist 38장 — dubi 14 · bli 14 · 곰돌이 10).
