...
**1단계 백엔드 구현 완료(§14)**, **1단계 프론트 구현 완료(§16, 2026-09-08)**,
**2단계 무드 연동 완료(§18, 2026-09-08)**.

부수 트랙으로 **손그림 비개구리 스티커**가 §15(실험) → **§17(세트 10종 확정·에셋 편입 완료)** 로 갈라져
나갔다. 이름은 같은 문서에 있지만 **기능이 다르다** — 우리 이모지는 사진으로 런타임 생성(원가 있음, PRO),
비개구리는 번들 PNG(원가 0, 전부 무료)다.
...
## 16. 1단계 프론트 구현 기록 (2026-09-08)

§7 표의 1단계를 전부 붙였다. 커밋 셋: 데이터 계층(`dcf1f11`) / 생성 화면(`8fa349a`) /
채팅 연결(`46aa2d9`). 검증은 `npm run typecheck` + `npm run build:web` 통과.

### 설계와 달라진 점

- **`api/coupleEmoji.ts` 는 `runAiJob` 으로 감싸지 않는다.** 다른 AI 기능처럼 "평범한 Promise"
  로 만들면 화면이 폴링 주기를 못 쥐고, §7 이 요구한 "6칸이 하나씩 채워지는" 대기 UI 를 그릴 수
  없다. 그래서 `generate` 는 `jobId` 만 돌려주고, 화면이 `awaitAiJob`(성패 판정)과 4초 주기
  `list()` 재조회(진행률)를 **동시에** 돌린다.
- **원본 업로드 경로를 분리했다.** `uploadImage` 는 서명 실패 시 unsigned preset 으로 폴백하는데,
  그 경로로 올리면 폴더가 달라 서버가 `INVALID_PHOTO_URL` 로 거절한다. `uploadImageWithSignature`
  를 따로 빼고 폴백을 두지 않았다. 서명 발급 → 업로드 → 접수 순서는
  `startCoupleEmojiGeneration` 한 함수 안에 가뒀다(순서를 틀릴 여지를 없앤다).
- **크롭은 `AvatarCropSheet` 를 그대로 재사용했다.** 정사각 512 JPEG 출력·업로드는 확정 후 한 번
  이라는 성질이 그대로 맞았다. 새로 만들지 않았다.
- **`store/coupleEmojiStore.ts` 는 `CoupleEvent` 를 스스로 듣지 않는다.** 소켓 구독은
  `chatSocket.register` 가 **목적지당 핸들러 하나**라, 스토어가 전역으로 구독하면 홈 화면의
  커플 채널 구독을 덮어쓴다. 그래서 구독은 화면(`ChatRoomScreen`)이 `useFocusEffect` 범위로
  갖고, 스토어는 `load(true)` 만 노출한다.
- **탭 라벨은 "우리 이모지"** — 기존 두 탭(이모지·이모티콘)과 헷갈릴 여지가 있지만, 기능 이름을
  줄이면 무엇인지 알 수 없다. 빈 상태에서는 탭 대신 설명 카드가 패널을 채운다.

### 아직 안 한 것

- **실기기 확인(§11-4)**: 생성 → 트레이 → 전송 → 상대 화면 갱신 → 삭제 → 관계 삭제 후
  Cloudinary 잔존. 코드 경로는 다 있으나 실제로 한 바퀴 돌려본 적이 없다. **배포 전 필수.**
- **원가 실측(§8)**: `AI_COUPLE_EMOJI` 월 5세트는 여전히 임시값이다.
- **2단계 무드 연동(§7 아래)**: `mood_statuses.couple_emoji_id` 마이그레이션부터.
- 웹은 번들·타입만 확인했다. 웹 런타임은 이 기능과 무관한 `requireNativeComponent` 문제로
  현재 뜨지 않는다(@sentry·@stream-io·react-native-webview 중 하나).

## 17. 비개구리 세트 확정 — 번들 에셋 편입 (2026-09-08)

§15 가 남긴 "남은 결정"(세트 구성 · 무료/PRO 배분 · 배경 순백 통일 · 크기 정규화)을 전부 확정하고
에셋으로 넣었다. **이 트랙은 우리 이모지와 별개다** — 런타임 생성이 아니라 번들 PNG 이므로 원가가 0이고
Gemini·PlanGuard·Cloudinary 를 하나도 지나지 않는다. 기존 `MessageType.STICKER` + `StickerImage` 경로에
그대로 얹혔다(`CHAT_EMOTICON_EXPANSION_ANALYSIS_2026-09-07.md` §7 (B) 경로).

### 결정

| 항목 | 결정 |
|---|---|
| 캐릭터 이름 | **비개구리** 유지 → 코드 prefix `BIGAE_`, 라벨은 감정만("화났어") |
| 세트 구성 | **10종**. 실험 11장(감정 10 + 씰룩씰룩) 중 "메롱" 제외 |
| 무료/PRO | **전부 무료.** 번들 에셋이라 원가가 0이고, 자체 캐릭터의 첫인상을 잠금 배지로 시작하지 않는다 |
| 상호작용(꽃단장·안기·이뻐해주기) | 스케치가 아직 없어 이번 범위 밖. 두 마리 구도로 그리면 `sketch.mjs` 로 바로 뽑는다 |

10종(트레이 순서 = 쓰임새 많은 것부터): `BIGAE_LOVE` 좋아좋아 · `BIGAE_EXCITED` 신났어 ·
`BIGAE_LAUGH` 하하하 · `BIGAE_WIGGLE` 씰룩씰룩 · `BIGAE_SULKY` 시무룩 · `BIGAE_ANGRY` 화났어 ·
`BIGAE_DASH` 흥, 간다 · `BIGAE_CRYING` 엉엉 · `BIGAE_GLOOMY` 축 처짐 · `BIGAE_SLEEPY` 잘자.

**"메롱"을 뺀 이유**: 테두리가 흰색이 아니라 보라색으로 나와 나머지 9장과 그림체가 어긋났고,
"좋아좋아"가 이미 혀 내밀기를 다뤄 의미도 겹쳤다. 재생성해서 11장을 맞추는 선택지도 있었지만
(장당 약 0.04 USD), 한 장이 세트 전체를 싸구려로 보이게 하는 쪽이 손해가 크다.

### 정규화 — 배경만 투명하게 만들 수 없었다

`scripts/couple-emoji-experiment/normalize_stickers.py`. 목표 규격은 기존 `love_bear.png` 과 같은
**360×360 RGBA 투명 배경**인데, 생성물은 1024 RGB 이고 배경이 순백이 아니다(252,250,251 근방).

문제는 **배경과 캐릭터를 감싼 흰 스티커 테두리가 둘 다 흰색이고 서로 붙어 있다**는 것이다. 가장자리에서
flood fill 하면 배경을 지나 테두리까지 그대로 먹어 버린다. 임계값을 좁혀 가르려 해도 배경 252 vs
테두리 255 로 3단계 차이라 안정적이지 않다. 그래서 **테두리를 지운 뒤 다시 그린다**:

1. 가장자리에서 도달 가능한 흰색 덩어리(배경 + 원래 테두리)를 지운다 — 눈 흰자·이불처럼 캐릭터
   **내부**의 흰색은 가장자리에서 도달할 수 없으므로 살아남는다
2. 남은 캐릭터를 18px 팽창시켜 흰 테두리를 새로 칠한다 → 10장이 같은 두께가 된다
3. 내용에 맞춰 자르고 정사각 여백(92%)을 준 뒤 360 으로 줄인다

부수 효과가 오히려 이득이었다 — 장마다 다르던 테두리 두께가 한 규격으로 통일됐다. 결과 67~112KB
(`love_bear.png` 115KB 와 같은 대역).

### 동기화 테스트를 추가했다 (Gradle 함정 포함)

`StickerImage` ↔ `STICKER_IMAGES` 는 양쪽 주석이 서로 "여기 추가하면 거기도 추가할 것"이라고만 적어
두고 **지켜주는 장치가 없었다**. `StickerImageSyncTest` 로 셋을 대조한다 — 코드 집합, 한국어 라벨
(알림 미리보기는 백엔드 label, 트레이 툴팁은 프론트 label 이라 어긋나면 화면마다 다른 이름이 보인다),
`require()` 경로의 에셋 파일 존재(tsc 는 이걸 잡지 못하고 번들에서 터진다).

**그 과정에서 기존 동기화 테스트 전체에 걸린 문제를 하나 찾았다.** 프론트 소스가 Gradle `test` 태스크의
입력이 아니라서, **프론트만 고치면 `UP-TO-DATE` 로 건너뛰고 어긋난 채 로컬에서 조용히 통과했다**
(`PlanFeatureSyncTest`·`PolicyVersionSyncTest` 도 같은 약점이었다). `build.gradle` 에 `inputs.files` 로
세 경로(`types/index.ts`·`legal.ts`·`stickerImages.ts`)를 선언해 프론트 변경만으로도 다시 돌게 했다.
일부러 라벨을 어긋내 재현·확인했다. CI 는 새 체크아웃이라 원래 늘 돌던 부분이다 —
**새 동기화 테스트를 추가하면 그 경로도 `inputs.files` 에 같이 추가할 것.**

**검증**: 백엔드 H2 전체 586건 통과(기존 584 + 새 테스트 2). 프론트 `typecheck`·`build:web` 통과,
`dist` 에 10장 모두 번들 확인.

### 남은 것

- **PRO 장을 만들 때**: `StickerImage` 에 `premium` 필드를 만드는 것만으로는 서버가 막지 않는다.
  `ChatService.send` 의 PRO 판정 조건이 `StickerPack`·`AnimatedSticker` 만 보므로 거기에
  `StickerImage` 도 넣어야 한다(양쪽 주석에 남겼다).
- 상호작용 스티커(꽃단장·안기·이뻐해주기) — 두 마리 구도 스케치 대기. 기존 가상 터치
  (손잡기·토닥임·포옹·뽀뽀)의 결과 화면과 짝을 이룰 수 있다.
- 실기기에서 트레이·말풍선 확인. 코드 경로는 기존 `STICKER` 그대로라 위험이 낮지만 돌려본 적은 없다.

## 18. 2단계 무드 연동 구현 기록 (2026-09-08)

§7 아래 "2단계 — 무드 연동"을 붙였다. 우리 이모지를 **무드 상태로 걸 수 있다** — 채팅 스티커는
보내는 순간이 끝이지만 무드는 홈에 하루 종일 떠 있어서, §7 이 "본 무대일 수 있다"고 본 쪽이다.

### 선택지가 너무 늘어나는 문제 (사용자 지적)

착수 직후 사용자가 지적했다: "무드에 나오는 표정들이 너무 많지 않아?" 실제로 세보면 막힌다 —
PRO 가 월 5세트까지 만들 수 있어 우리 이모지만 최대 30장, 유니코드 24종(무료 12 + PRO 12)을
더하면 **54개**다. `MoodPicker` 에는 이미 "확장팩까지 24종이라 작은 화면에서는 넘친다"는 주석이
있고, 12종 원칙 자체가 `moodEmojis.ts` 의 **"처음부터 다 만들면 선택 마비만 생긴다"**(Obimy 60여 종을
12종으로 줄인 근거)에서 나왔다. 그냥 얹으면 그 원칙과 정면으로 부딪힌다.

**두 축으로 줄여 6장으로 고정했다.** 열쇠는 `couple_emojis.subject_user_id`(누구 얼굴인가)였다.

1. **내 얼굴만** — 무드는 "내 기분"이므로 `subjectUserId === 나` 인 세트만 후보다. 애인 얼굴 세트는
   애초에 무드 선택지에 들어오지 않는다.
2. **최신 한 벌만** — 그중 가장 최근 `batchId` 하나. 세트를 몇 벌 만들어도 여기는 6장이다.

결과: 무료 사용자 12 + 6 = 18. 대안으로 "선택지를 아예 안 늘리고 토글 하나로 유니코드를 얼굴로
바꿔 그리기"도 검토했는데, 대응 무드가 없는 **사랑** 감정이 버려지고 발견성이 떨어져 접었다.

### 설계에서 정한 것

| 항목 | 결정 | 이유 |
|---|---|---|
| 컬럼 | `mood_statuses.couple_emoji_id BIGINT NULL` + FK (**V81**) | `emoji` 가 `VARCHAR(10)` 이라 URL 이 안 들어간다. 컬럼을 늘리지 않고 참조를 붙였다 |
| `emoji` | **NOT NULL 유지.** 우리 이모지를 골라도 감정 대역을 함께 저장 | 푸시 미리보기가 `"지금 기분: " + emoji` 로 그 값을 그대로 읽고, 이 컬럼을 모르는 화면도 계속 동작해야 한다. `StickerPack` 이 코드가 아니라 이모지 문자를 저장하는 것과 같은 이유 |
| 대역 선정 | `CoupleEmojiEmotion.moodEmoji()` = 😤😊🥳😢😴🥰 — **전부 기본 12종 안에서** | 확장팩(PRO) 이모지를 쓰면 `MoodService.set` 의 `MoodPack.isPremium` 판정에 걸려 **무료 사용자가 자기 이모지를 무드로 걸 때 402** 가 난다. 테스트로 고정했다 |
| 요청 | `emoji` 이거나 `coupleEmojiId`(둘 중 하나). 프론트는 `MoodChoice` 유니온 | `emoji` 에 `@NotBlank` 를 걸 수 없어 서비스에서 검증한다 |
| `emoji` 신뢰 | 우리 이모지면 **클라이언트 값을 버리고** 감정에서 다시 채운다 | 엉뚱한 값(PRO 이모지 등)을 보내 판정을 흔드는 경로를 없앤다 |
| PRO 판정 | **하지 않는다** | 만들 때 이미 `AI_COUPLE_EMOJI` 로 과금·게이팅했고, 커플 공용이라 무료인 상대도 걸 수 있어야 한다(채팅 전송과 같은 규칙) |

### URL 을 복사하지 않은 것이 핵심이다

채팅 메시지는 `image_url` 을 **복사**한다(지난 메시지가 조인 없이, 숨긴 뒤에도 그려져야 해서).
무드는 **반대로 참조만 둔다.** 상대가 "내 얼굴 그만 써" 하고 이모지를 지웠는데 홈 배지로 하루 종일
남아 있으면 §9 의 약속이 깨지기 때문이다. 그래서 읽을 때 `DeletedAtIsNull` 로 확인해 숨겨졌으면
**유니코드로 되돌린다** — 무드 행은 원장이라 지우지 않고, 그리는 방식만 바뀐다.

실시간 반영에는 **추가 배선이 필요 없었다.** `subscribeCouple` 이 이벤트 종류를 가리지 않고
`refresh()` 를 부르고(`HomeScreen`), 서버가 이모지 삭제 때 `CoupleEvent.COUPLE_EMOJI` 를 발행하므로,
무드 응답의 `imageUrl` 이 그 시점에 null 로 내려온다.

### Purger

`RelationRecordPurger` 는 이미 `mood_statuses` 를 `couple_emojis` 보다 먼저 지우고 있어 순서가 그대로
맞았다. 다만 **이제 FK 때문에 그 순서가 필수**가 됐으므로(뒤집으면 관계 삭제가 FK 위반으로 실패)
양쪽에 주석으로 고정했다.

**검증**: H2 전체 591건 통과(`MoodFlowTest` 9건 — 신규 5건: 이미지 동반 조회, 대역이 전부 무료인지,
숨김 시 유니코드 폴백, 남의 관계 이모지 거절, 둘 다 없으면 거절). **PostgreSQL 전체 591건도 통과**하고
V81 의 컬럼·FK 가 실제로 적용된 것을 `\d mood_statuses` 로 확인했다(마이그레이션을 건드렸으므로
CLAUDE.md 6절). 프론트 `typecheck`·`build:web` 통과.

### 남은 것

- **무드 캘린더(히스토리)는 아직 유니코드만 그린다.** 응답에 `imageUrl` 이 실리므로 같은 분기를
  넣으면 되지만, 지난 무드는 그 시점 이모지가 이미 지워졌을 수 있어 폴백이 더 자주 걸린다.
- **무드 피커에 우리 이모지가 없으면 섹션 자체가 안 나온다.** "만들기" 안내를 넣지 않은 건 생성
  진입점이 채팅 트레이 한 곳이어서다 — 여기 또 두면 같은 기능의 입구가 둘로 갈린다. 다만 무드에서만
  이 기능을 만나는 사용자는 존재를 모른다. 1단계 반응을 보고 결정.
- 실기기 확인은 §16 의 목록에 이 경로(무드 걸기 → 홈 배지 → 상대 화면 → 이모지 삭제 후 폴백)를 더한다.

## 19. 트레이를 캐릭터별로 나눴다 (2026-09-14)

§17 로 비개구리 10종이 들어오면서 "이모티콘" 탭의 이미지 스티커가 20장이 됐는데, 화면은 그대로
**"캐릭터" 이름표 하나 밑에 20장을 이어 붙이고 있었다.** 두 세트에 같은 감정이 여섯 개나 겹쳐
있어서(신났어·하하하·시무룩·화났어·엉엉·잘자) 그림만 바뀐 같은 스티커가 두 번 나오는 것처럼
보였고, 그림체가 다른 두 세트가 줄바꿈 없이 붙으면 세트가 아니라 잡동사니로 읽힌다.

**구획을 화면이 아니라 카탈로그가 들게 했다.** `stickerImages.ts` 를 평평한 배열에서
`STICKER_CHARACTERS`(캐릭터 → 스티커 목록)로 바꾸고, `ChatRoomScreen` 은 그것을 순회하며 캐릭터마다
이름표를 하나씩 찍는다. 캐릭터가 늘어도 화면은 고치지 않는다. `STICKER_IMAGES` 는 `flatMap` 결과로
남겨 뒀다 — 말풍선·알림 미리보기는 캐릭터를 모르고 코드로만 찾으므로(`stickerImageOf`) 그 경로를
건드릴 이유가 없었다.

**비개구리를 위에 둔다.** 앱 아이콘이 된 마스코트이고(`docs/APP_ICON_BIGAE_REVIEW_2026-09-10.md`)
사용자 손그림에서 나온 자체 캐릭터라 이 앱에만 있는 쪽이다. 곰돌이는 참조 그림 한 장에서 변주를
뽑은 세트다(§17 · `variants.mjs`).

**주의 — `StickerImageSyncTest` 의 정규식.** 그 테스트는 `{ code: 'X', label: 'Y', source: require('…') }`
한 항목을 정규식으로 긁어 백엔드 enum 과 대조한다. 세 필드의 **이름과 순서**를 바꾸면 파싱이 0건이
되어 테스트가 `isNotEmpty()` 에서 죽는다. 바깥 구조(캐릭터로 묶는 것)는 파일 전체를 훑으므로 자유롭다.
양쪽 주석에 남겼다. 백엔드 `StickerImage` 에는 캐릭터 개념이 없다 — 서버는 코드로 라벨 하나만 찾으면
되고 enum 선언 순서는 화면에 영향을 주지 않는다는 것도 같이 적었다.

검증: `npm run typecheck`, `StickerImageSyncTest` 통과.

### 남은 것 — 비개구리 2차 스케치

사용자가 새 스케치 한 장(흰 줄 노트, 한 페이지에 8종 내외)을 올렸다. §15 와 같은 경로
(`crop` → `sketch.mjs` → `normalize_stickers.py`)로 바로 태울 수 있지만 **스케치 원본이 저장소에
없어서 멈췄다** — `scripts/couple-emoji-experiment/photos/` 는 `.gitignore` 대상이고 채팅 첨부는
디스크에 남지 않는다. 사진을 그 폴더에 넣으면 이어서 진행한다. 감정 후보와 라벨은 그때 확정한다.
