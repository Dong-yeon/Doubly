...
## 4. 곁가지 — 백엔드가 앱보다 앞서 있다 (점검 완료 2026-09-14)

`main` 푸시로 Railway 가 배포돼, 지금은 **백엔드가 스토어 앱 바이너리보다 앞선 상태**다.
구 앱이 보내지 않는 필드를 백엔드가 요구하면 그 자리에서 깨지므로, 빌드를 미루기로 한 이상
이 점검은 미룰 수 없었다. **`11bdfab`(스토어 라이브) → `a67fff7` 구간을 전수 확인했고,
깨지는 지점은 없다.**
...
| 깨지는 모양 | 확인 방법 | 결과 |
| --- | --- | --- |
| 응답에서 필드가 사라짐 (구 앱이 읽던 값이 `undefined`) | 바뀐 응답 DTO 5종의 record 컴포넌트를 구·신 비교 | 사라진 필드 **0** |
| 요청에 필수 필드가 늘어남 (구 앱이 안 보냄 → 400) | `*Request.java` 에 새로 붙은 `@NotBlank`·`@NotNull`·`@Size` 전수 | 4건 전부 **새 DTO**(`GameReactionRequest`·`GuessRequest`·`StartCatchMindRequest`) — 구 앱이 부르지 않는 엔드포인트다 |
| 동작이 좁아짐 (되던 게 막힘) | 게이팅 변경 검토 | 유일한 변경이 `StickerPack` 폐지인데 **넓어지는 쪽**이다 — 구 앱이 시즌 이모지를 보내면 예전엔 402 였고 이제 통과한다 |

DTO 변경은 294줄 추가 / 6줄 삭제였는데, 그 6줄은 전부 record 파라미터 목록에 필드를
끼우면서 줄이 다시 흐른 것이고 실제 제거는 없다.

새 마이그레이션 V89~V93 은 전부 **테이블·컬럼 추가**다(`chat_messages.client_message_id`,
`ai_usage_logs`, 오목 수 이력, 스도쿠 일일 퍼즐, 캐치마인드). 구 앱이 읽는 컬럼을 지우거나
NOT NULL 로 조이는 것이 없다.

한계: 여기서는 **정적 확인만** 했다. 배포된 Railway 인스턴스에 직접 요청을 보내 확인하지는
못했다(이 작업 환경의 egress 정책이 `fitto-production.up.railway.app` 을 막는다).
