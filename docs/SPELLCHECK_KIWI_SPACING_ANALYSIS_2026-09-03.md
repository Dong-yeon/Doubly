...
수 표현에 ❌ 를 달았지만, **맞춤법상으로는 Kiwi 가 맞다** — 단위명사는 띄어 쓰는 게
원칙이다. 문제는 정확성이 아니라 커플 앱에서 "천일"이 하나의 말로 쓰인다는 것,
즉 **사용자가 원치 않는 변경**이라는 점이다. 그래서 사용자 사전으로 억지로 막는 대신
UX 로 풀었다(§4-3). 사전이 필요해질 때를 대비해 `kiwi_builder_add_word()` /
`kiwi_builder_load_dict()` 를 부를 자리는 builder 흐름에 만들어 뒀다.
...
**번들로 결정했다**(사용자, 2026-09-03). 그래서 `loadSpacing()` 은 인자를 받지 않고
APK 안 `assets/kiwi/` 를 직접 읽는다(§4-1). 나중에 내려받기로 바꾼다면 asset 스트림
대신 파일 경로를 받는 갈래를 하나 더 두면 되고, 그 위(훅·UI)는 그대로다.

메모리 241MB 는 화면 수명에 묶어 해결했다 — 리뷰·일기 화면에 들어올 때 올리고 나갈 때
내린다(`useSpacingFix` 의 `useFocusEffect`). 로드 3~4초는 화면 진입과 함께 백그라운드로
시작하므로 사용자가 버튼을 누를 땐 대개 이미 준비돼 있다.
