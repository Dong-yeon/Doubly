...
> **[2026-09-15] 0~3단계 완료.** 0단계 선행 병합(`46b1ba5`), 1단계 백엔드 4소스 API,
> 2단계 럽바디 탭, 3단계 우리 탭이 모두 구현됐다. 아래 표의 파일 목록은 설계 기록으로
> 남긴다 — 실제 구현에서 달라진 점은 이렇다.
>
> - `DietScreen` Props 만 `HealthStackParamList` 로 넓혔다(운동 라우트로 이동해야 해서).
>   나머지 화면 20여 개는 좁은 파람리스트 그대로 등록된다 — 교집합 타입이라 통과한다.
> - 운동 홈에 따로 있던 재개 카드를 체크인 카드에 합쳤다(두 화면이 갈라지지 않게).
> - 홈에서 기록 화면으로 갈 때 `initial:false` 를 식단 기록에도 붙였다(같은 튕김 버그).
> - `workouts` 에는 **status 컬럼이 없다** — 5-4 의 `status='COMPLETED'` 필터는 해당
>   사항이 없다. 끝내지 않은 운동은 행이 아니라 기기 초안(`activeWorkoutStore`)이라
>   이 테이블의 행은 전부 완료분이다(`WorkoutRepository.findPhotosForFeed` 주석).
> - 타임라인 매퍼가 운동 `imageUrl` 을 null 로 채워서(카드가 운동 사진을 안 그린다)
>   사진첩은 원본 엔티티에서 URL 을 직접 읽는다 — 테스트가 잡아낸 함정이다.
> - 홈 스택에도 등록되는 `FeedCompose`·`TripAlbum` 은 우리 탭 쪽 딥링크 경로를 나눴다
>   (`album/new`·`album/trips/:tripId`).
