# 하단 탭 재구성 2차 — 운동 탭 자리에 앨범 (2026-09-14)

> **한 줄**: 운동·식단 탭을 합치는 것 자체는 이득이 없다. 대신 **운동을 식단 탭 안 체크인 카드로
> 흡수해 탭 한 칸을 비우고, 그 자리에 모든 사진 소스를 모은 "앨범" 탭을 넣는다.**
> 지금의 사진첩(피드 사진만 보이는 3열 그리드)을 그대로 승격하는 것은 아니다.
>
...
3. → "운동 탭 자리에 앨범을 넣을까?"
4. → 조건부 찬성. 조건은 **모든 사진 소스 통합**. 사용자 확정(2026-09-14).
...
| 2 | **Health** | **건강** | `silverware-fork-knife` → 추후 재검토 | **`HealthStackNavigator`** — 기존 Diet 스택 + Workout 스택 전체 |
| 3 | Chat | 채팅 | `chat-outline` | 변경 없음 |
| 4 | **Album** | **앨범** | `image-multiple-outline` | **`AlbumStackNavigator`** — 신규 |
| 5 | Place | 럽슐랭 | `crown` | 변경 없음 |

라벨 "건강"은 1차 재구성 전 우산 이름을 되살린 것이다. 운동·식단·몸 변화·물·단식이 함께 사는 탭에
"식단"은 좁고 "오늘"은 히스토리·통계까지 품는 탭에 어색하다. 이건 라벨 한 줄이라 나중에 바꾸기 쉽다.

### 5-2. 건강 탭 — 운동을 어떻게 흡수하나
...
- **세그먼트 토글은 만들지 않는다.** 식단 메인이 곧 건강 메인이고, 운동은 그 안의 카드 하나다.
...
| `navigation/MainTabNavigator.tsx` | `TAB_META` Health 추가, Workout·Diet 제거, `HIDE_ACTIVE_WORKOUT_BAR_ON` 유지 |
...
커밋 2개: `refactor(nav): 운동 스택을 식단 스택에 합쳐 건강 탭 하나로` / `feat(diet): 건강 메인 상단에 운동 체크인 카드`

### 3단계 — 프론트: 앨범 탭 신설 (탭 4→5)
...
| `navigation/MainTabNavigator.tsx` | 4번째에 Album |
...
커밋 2개: `feat(album): 앨범 탭 신설 — 4소스 그리드 + 작년 오늘 + 여행 앨범` / `refactor(home): 우리 기록·사진첩을 앨범 탭으로 이관`
...
3. `PLAN.md` 하단 탭 재구성 절에 상태 갱신, `README.md` 탭 설명, `docs/QA_CHECKLIST.md`에 앨범 항목.
...
- **건강 탭 첫 화면 무게** — `DietScreen`이 1072줄에 카드 하나가 더 얹힌다. 이번엔 얹기만 하고, 물·단식·목표
  카드 정리는 별건으로 남긴다.
...
- 건강 탭 라벨·아이콘 최종 확정 — 라벨 한 줄이라 실기기에서 보고 결정
