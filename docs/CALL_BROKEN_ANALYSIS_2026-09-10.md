...
**이 한 줄이 §4 의 두 후보를 모두 배제한다.** 연결이 됐다는 것은  도,
 도,  도, 세션 생성도 전부 지났다는 뜻이다 —
설정·게이팅·한도 층은 통과했다.
...
|---|---|
|  권한 | 마이크를 못 잡는다 → **소리 없음** |
|  의 네이티브 설정 | 오디오 세션·코덱이 안 잡힌다 |
|  의 포그라운드 서비스 선언 | 안드로이드가 통화를 곧 정리한다 → **금방 꺼짐** |

 로 확인한 병합 매니페스트에는 이 권한들이 다 있다(§1) —
즉 **소스는 맞고 빌드 산출물이 다르다**. 지금 기기에 깔린 앱이 빌드 26·27 이면 설명이 그대로 들어맞는다.

### 다음 한 수

 수정() 이후로 **새로 빌드**해서 같은 증상이 남는지 본다. JS 만
바뀐 게 아니므로 **EAS Update 로는 안 되고 빌드여야 한다.** 빌드 로그에서
 를 반드시 확인한다( 면 같은 사고가 반복된 것이다).
