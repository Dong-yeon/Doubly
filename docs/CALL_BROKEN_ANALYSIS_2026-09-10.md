...
> **[2026-09-23] 이 문서의 결론(§5 PREBUILD skipped)은 틀렸다.**
> 진짜 원인은 **런타임 마이크 권한을 요청하는 코드가 앱에 하나도 없었던 것**이다.
> 실기기 로그에 `Mic init failed: SecurityError: Permission denied` 가 그대로 찍혀 있었다.
> 아래 §1 에서 매니페스트에 `RECORD_AUDIO` 가 **선언**된 것을 확인하고 권한 층을 배제했는데,
> 선언과 사용자 **허용**은 다른 문제다. 전말: [CALL_MIC_PERMISSION_2026-09-23.md](CALL_MIC_PERMISSION_2026-09-23.md).
...
## 3-1. 증상 확인 — 결론이 좁혀졌다 (사용자 확인)

> "통화는 연결됐는데 금방 꺼지고 아무 소리도 나지 않았어."

**이 한 줄이 §4 의 두 후보를 모두 배제한다.** 연결이 됐다는 것은 `/calls/token` 도,
`callStore.init()` 도, `requireConfigured()` 도, 세션 생성도 전부 지났다는 뜻이다 —
설정·게이팅·한도 층은 통과했다.

깨진 곳은 **네이티브 미디어 층**이다. 신호(시그널링)는 Stream 서버를 거치는 네트워크
일이라 JS 만으로 되지만, <b>소리</b>는 기기의 마이크·오디오 세션·WebRTC 네이티브 모듈이
있어야 한다. "연결은 되는데 소리가 없고 곧 끊긴다"는 정확히 그 조합이 빠졌을 때의 모습이다.

그래서 **§5(PREBUILD skipped)가 유일하게 남은 유력 원인**이 된다. prebuild 를 건너뛴
빌드는 app.json 을 읽지 않으므로 config plugin 이 적용되지 않고, 통화에 필요한 것들이
통째로 빠진다:

| 빠지는 것 | 결과 |
|---|---|
| `RECORD_AUDIO` 권한 | 마이크를 못 잡는다 → **소리 없음** |
| `@config-plugins/react-native-webrtc` 의 네이티브 설정 | 오디오 세션·코덱이 안 잡힌다 |
| `@stream-io/video-react-native-sdk` 의 포그라운드 서비스 선언 | 안드로이드가 통화를 곧 정리한다 → **금방 꺼짐** |

`expo config --type introspect` 로 확인한 병합 매니페스트에는 이 권한들이 다 있다(§1) —
즉 **소스는 맞고 빌드 산출물이 다르다**. 지금 기기에 깔린 앱이 빌드 26·27 이면 설명이 그대로 들어맞는다.

### 다음 한 수

`.easignore` 수정(`423ccc6`) 이후로 **새로 빌드**해서 같은 증상이 남는지 본다. JS 만
바뀐 게 아니므로 **EAS Update 로는 안 되고 빌드여야 한다.** 빌드 로그에서
`PREBUILD:success` 를 반드시 확인한다(`PREBUILD: skipped` 면 같은 사고가 반복된 것이다).

그래도 소리가 없다면 그때는 기기 설정의 마이크 권한과 Stream 대시보드의 코덱 설정을 본다.
