...
### 1단계 — 큰 화면 셸 — **거의 완료** (남은 것은 아래 "미완" 표)

> 6절 1번 확정: **기본 형태는 420~480px 폭의 작은 세로 창.** 2열 레이아웃은 만들지 않는다.
> 최대 폭·사이드 레일은 사용자가 창을 넓혔을 때 깨지지 않게 하는 상한선이지, 넓은 창을
> 겨냥한 별도 설계가 아니다.

구현하면서 지키기로 한 **불변식 하나**: *화면이 실제로 쓰는 폭 == `layout.shellMaxWidth`(640)*.
이게 깨지면 폭을 직접 계산하는 화면(온보딩 페이저 등)이 조용히 어긋난다 — 실제로 셸에 좌우
테두리를 그었을 때 640 → 638 이 되어 페이저가 장마다 2px 씩 밀렸다. 아래 설계는 전부 이
불변식에서 나온다.

- [x] **앱 셸 컨테이너** (`components/AppShell.web.tsx`) — 창 > 640 일 때 콘텐츠를 640 으로
  묶고 가운데 정렬. 작은 창에서는 래퍼를 만들지 않는다. 토스트·다이얼로그도 안에 넣었다.
  네이티브는 `AppShell.tsx` 가 통과시킨다.
  - 화면마다 다른 폭(채팅 720 / 나머지 560~640)은 **버렸다.** 탭을 옮길 때마다 셸이 출렁여
    창 크기가 바뀌는 것처럼 보인다. 한 값으로 고정하는 편이 창처럼 읽힌다.
  - `hooks/useContentWidth` 신설. 격자 셀·페이저 폭을 `useWindowDimensions` 로 계산하던
    7곳을 옮겼다. 안 옮기면 640 셸 안에 1024 짜리 페이지가 그려진다.
- [x] **하단 탭 → 사이드 레일** — `tabBarPosition: 'left'`(bottom-tabs v7)로 내비게이터가
  가로 배치를 해주고, `CustomTabBar` 는 같은 `renderTab` 으로 방향만 바꾼다.
  - 경계는 **1024 가 아니라 `shellMaxWidth + railWidth`(728)** 하나로 뒀다. 셸이 640 고정인
    이상 창이 800 이든 1400 이든 콘텐츠 폭은 같아서, 그 사이에 경계를 더 두면 임의가 된다.
  - 레일은 셸 **안**이 아니라 셸을 넓혀서 단다(640 + 88). 안에서 자리를 빼앗으면 콘텐츠가
    552 로 줄어 불변식이 깨진다. "레일이 실제로 붙었는지"는 `components/shellRail.ts` 로
    탭 내비게이터가 셸에 알린다 — 창 폭만 보고 넓히면 레일이 없는 온보딩이 728 이 된다.
  - 채팅방에서 탭바를 감추던 규칙은 레일에 적용하지 않는다(그 규칙의 이유는 키보드다).
  - `ActiveWorkoutBar` 에 `compact` 추가 — 가로 바가 88px 레일에 안 들어간다.
- [x] **`SwipeBackView`** — `utils/pointer.ts` 의 `isCoarsePointer()` 로 마우스일 때만 끈다.
  `Platform.OS === 'web'` 로 가르면 모바일 브라우저의 스와이프까지 죽는다.
- [x] **PWA manifest** — `public/manifest.json` + `public/index.html`(Expo 는 이 파일이 있으면
  템플릿으로 쓴다) + `scripts/generate-pwa-icons.cjs`.
  - **서비스 워커는 넣지 않았다.** 크롬은 108(모바일)·112(데스크톱)부터 설치 요건에서 서비스
    워커를 뺐다 — 설치에는 필요 없다. 오프라인 캐시는 리프레시 토큰 회전과 엮여 별건이다.
- [x] **모달·시트** — 공용 `Sheet` 는 PC 에서 하단 시트를 가운데 다이얼로그(maxWidth 480)로
  바꾼다. 웹 모달은 `position:fixed` 라 셸이 아니라 뷰포트를 덮는데, 가운데는 그래도 맞지만
  하단은 셸에서 떨어진 1024px 바가 바닥에 붙는 꼴이 된다.

#### 1단계 미완 — 다음에 할 것

| 항목 | 왜 남겼나 |
| --- | --- |
| 하단 시트 11곳 | `Sheet` 를 안 쓰고 각자 `Modal` + `flex-end` 로 그린다(`MoodPicker`·`VoiceRecordSheet`·`QuickMealSheet`·`TripDetail`·`BodyMetric` 등). 웹 분기를 11번 복붙하는 대신 **먼저 `Sheet` 로 모으는 게 순서** — `Sheet` 주석이 원래 그 목적이다. 모으고 나면 PC 대응은 이미 끝나 있다 |
| 채팅 버블 절대 폭 | 셸이 640 을 물리므로 `maxWidth: '82%'` = 525px 로 이미 상한 안이다. 초안의 "1400px 버블"은 셸 이전 수치 — 지금은 할 일이 없어 넘긴다(셸 폭을 키우면 그때 필요) |
| 레일 실물 확인 | 로그인 이후 화면이라 자동 검증에서 눈으로 못 봤다. 실사용 1회 필요 |

### 2단계 — 키보드와 마우스 — **완료**(아래 한 건 제외)

- [x] **Enter 전송, Shift+Enter 줄바꿈** — 마우스일 때만(`isCoarsePointer()`). 모바일 브라우저에서
  켜면 화면 키보드의 줄바꿈 키가 전송으로 바뀐다.
  - **한글 조합 중 Enter 는 무시한다.** "안녕"을 치는 도중의 Enter 는 조합을 확정하는 키라,
    전송으로 받으면 마지막 글자가 잘린 채 나간다(`isComposing` / `keyCode 229`).
  - `preventDefault()` 는 줄바꿈만 막는 게 아니라 RNW 의 뒤따르는 기본 처리(`onSubmitEditing`
    + blur)까지 건너뛴다 — 덕분에 전송 후 입력창 포커스가 그대로 남는다.
- [x] **Ctrl+V 붙여넣기 · 드래그앤드롭** (`hooks/useImageDrop.web.ts`) — 둘 다
  `File` → `URL.createObjectURL` 로 끝나고, 그 uri 는 피커가 주는 것과 똑같이 그려지고
  업로드된다. 바로 보내지 않고 **기존 미리보기**로 들어온다(잘못 붙여넣은 스크린샷은 되돌릴 수
  없다). 끄는 동안 "여기에 놓으세요" 판을 띄운다 — 눌러볼 버튼이 없는 상호작용이라 표시가
  없으면 손을 놓기 전에 포기한다.
  - 클립보드 스크린샷은 **무압축 PNG + 화면 해상도 그대로**다(피커는 quality 0.7 로 이미
    압축해서 준다). `shrinkUnknownImage` 로 1600px 까지만 줄인다 — 글자가 든 화면 캡처가
    많아 채팅 기본값 1024 는 과하다.
  - `blob:` uri 는 `releaseObjectUrl` 로 푼다. PC 앱은 탭을 며칠씩 켜 둔다.
- [x] **hover** — `utils/pointer.ts` 의 `isHovered()`(RNW 는 주지만 RN 타입엔 없다) + 탭 레일 ·
  피드 리액션 칩. 선택된 탭·이미 누른 칩에는 덧대지 않는다(이미 색으로 구분된다).
- [x] **이모티콘·스티커 피커 클릭 크기** — 이미 기준을 넘어 손댈 게 없었다(`stickerBtn`
  minWidth 40 / 이미지 32).
- [ ] **메시지 시각 hover 표시** — *하지 않기로 했다.* 시각은 지금 그룹의 마지막 메시지에만
  뜨는데, 이건 2026-09-01 에 "메시지마다 표시"를 **일부러 되돌린** 결과다(카톡과 같은 규칙).
  hover 로 되살리면 그 결정을 옆문으로 다시 여는 셈이고, 채팅 목록에 hover 마다 리렌더가
  붙는다. 필요하다고 판단되면 그때 별도로 다룬다.
...
| --- | --- | --- |
| 0. 웹 지키기 | 0.5 | **완료** |
| 1. 큰 화면 셸 | 3~5 | **완료**(하단 시트 11곳 제외) |
| 2. 키보드·마우스 | 1~2 | **완료** |
| 3. 알림·재접속 (배지까지) | 0.5~1 | 미착수 |
| 4. 보안 | 1~2 | 미착수 |
| **합계** | **6~10.5일** | |

**실사용 검증이 통째로 남아 있다.** 0~2단계는 전부 로그인 전 화면에서만 자동 검증했다
(타입체크·웹 번들 빌드·부팅 콘솔 에러 0·셸 폭 실측). 레일·Enter 전송·붙여넣기·드래그앤드롭은
**로그인 이후 화면이라 아직 아무도 눈으로 못 봤다** — 3단계보다 이게 먼저다.
