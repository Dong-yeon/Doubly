...
- **Web Push(VAPID)**: 백엔드에 웹 구독(endpoint·p256dh·auth) 저장 테이블 + 발송 경로 신규.
  기존 `notification` 패키지의 Expo 발송과 나란히 두고, 발송 팬아웃에서 기기 타입별로 분기.
  Purger 규칙(CLAUDE.md 4절)에 새 테이블 추가 필수. 프론트는 `push.ts` 의 웹 분기 6곳을
  Web Push 등록으로 교체.
...
- Google 로그인: `webClientId` 채우고 오리진 등록. 이메일 로그인이 이미 되므로 급하지 않다.
...
1. **창 형태**: "작은 세로 창(PC 카톡형, 420~480px 폭)"을 기본으로 갈지, "넓은 화면에서 2열(채팅 +
   오늘 위젯)"까지 갈지. 전자는 1단계에서 셸 컨테이너만으로 끝나고, 후자는 화면 설계가 추가된다.
   → 권장은 전자. 커플은 방이 하나라 2열이 줄 정보가 적다.
2. **알림 수준**: 탭 제목 배지로 시작할지, Web Push 까지 갈지(백엔드 테이블 추가).
3. **Google 로그인 웹 활성화** 여부 — 회사 PC 에서 비번 치는 게 싫다면 이게 답이다.
4. **CI 웹 검사 추가** — 이번 크래시 재발 방지. 이건 반대할 이유가 없어 0단계에 그냥 넣는다.
