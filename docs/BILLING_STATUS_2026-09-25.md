...
| 5 | ~~구매 복원 버튼~~ **완료(9/25)** — 플랜 화면 고지 줄에 "구매 복원" 링크(`restorePurchases`). 결과를 항상 말한다(복원됨/미반영/없음/연결 실패) | 애플 심사 3.1.1 이 눈에 보이는 복원 수단을 본다. 기기 교체·재설치 뒤 "PRO 였는데 사라짐" CS 도 여기서 끝난다 | — | `PlanScreen.tsx`, `iap.ts` |
...
## 6. 9/25 추가 — "결제했는데 계속 '반영 전' 알림" 의 원인과 수정

**증상**: 결제한 뒤 앱을 켤 때마다 "결제는 확인했어요. 반영이 조금 늦어지고 있어요"가 뜬다.

**원인(확정)**: `getAvailablePurchases()` 는 "아직 안 닫은 트랜잭션"이 아니라 **살아 있는 구독
전부**를 돌려준다(react-native-iap 문서). `initIap` 은 그 결과를 전부 `verifyAndFinish` 에 넣었고,
그 함수의 catch 하나가 **모든 실패**(이미 닫힌 트랜잭션을 또 닫는 실패·서버 미반영·401·네트워크)에
같은 문장을 띄웠다. 그래서 구독이 유효한 한 매 실행마다 반복됐다 — PRO 가 멀쩡해도, 서버가 정말
반영을 못 했어도 문장은 같았다.

**수정(`utils/iap.ts`)**:
- 처리한 구매를 저장소(`doubly.iapHandled`)에 기록 — 반영 확인(`done`)·미반영 안내(`warned`).
  앱 시작 경로는 `done` 인 구매를 건너뛰고, 안드로이드는 `isAcknowledgedAndroid` 도 `done` 으로 본다
  (이 수정 전에 닫은 구매도 걸러진다).
- 실패를 셋으로 나눈다: 서버 응답이 PRO 가 아니면 **같은 구매에 한 번만** 안내(방금 결제·복원은
  항상), 검증 요청 자체가 실패하면(401·네트워크) 앱 시작 경로에서는 조용히 다음 실행에 맡긴다,
  반영됐으면 닫는 호출이 실패해도(이미 닫힘) 반영으로 기록한다.
- "PRO가 시작됐어요!" 는 방금 결제했을 때와 처음 반영을 확인했을 때만.

**이 수정이 못 고치는 것**: 서버가 정말 반영하지 못한 경우(Railway 키 비어 있음·귀속 불일치·
샌드박스 만료). 그건 플랜 화면이 FREE 로 남고 "구매 복원"을 누르면 "반영이 늦어지고 있어요"가
다시 뜬다 — 그때는 서버 로그(`App Store 구독 상태를 조회하지 못함` / `연결할 수 없음` /
`Play 구독 …`)와 `subscriptions` 표를 본다. 안드로이드는 트랜잭션을 못 닫은 채 3일이 지나면
**자동 환불**된다.
