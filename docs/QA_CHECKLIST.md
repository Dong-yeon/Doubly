...
>
> **지금 가장 급한 건 따로 있다**: [iOS 전역 오버레이 회귀 체크리스트
> (2026-09-07)](IOS_OVERLAY_REGRESSION_CHECKLIST_2026-09-07.md) — 방금 앱 전체 확인창·
> 시트·로딩에 영향 가는 수정(`ed8e525`)이 실기기 검증 전이라 이 문서보다 먼저 본다.
...
| 열 수 (폭 적응, 2026-09-15) | 폰 3열 고정 · 태블릿·폴더블(≥600dp)은 칸 크기 기준으로 최대 6열 — `usePhotoGrid` | — | — | — | ☐ |
...
| 리액션 칩 토글 | `chatApi.react` | ❌ | ✅ toast | ✅ 메시지별 순차 전송 (2026-09-15) — 탭을 버리지 않고 줄을 세운다 | ☐ |
...
- ~~**[확정]** 리액션 칩에 락 없음~~ → **해소(2026-09-15)**. 락 대신 메시지별 약속 사슬로
  순서를 보장한다(`onReact`). 락으로 막으면 연타의 뒷 탭이 씹혀 "눌러도 반응 없음"이 된다
