...
### 화면 (frontend)
- `TripListScreen` — 다가오는(D-N)/여행 중/다녀온 여행 카드
- `TripFormScreen` — 생성/수정 겸용 (제목·날짜·메모·커버 사진)
- `TripDetailScreen` — 여행 정보 + 담긴 장소 목록(지도 핀) + 장소 담기/빼기
- ~~맛집 지도 상단에 "✈️ 여행" 진입 칩~~ → **홈 스택으로 이관** (아래 "여행 IA 이관" 참고)

> **여행 IA 이관 (2026-08)**: Trip 스위트(7화면)는 원래 장소(Place) 스택에 있었다 —
> "여행은 장소의 상위 집합"이라는 전제였다. 그러나 장소 탭이 **럽슐랭**(미식 가이드북)으로
> 리브랜딩되며 그 전제가 무너졌고(여행이 탭 정체성 밖으로 밀려 진입 버튼도 가려짐),
> 여행의 사용 리듬(계획→D-day→진행→회고의 간헐적 이벤트)은 일상 축적형 컬렉션인
> 럽슐랭보다 홈의 '우리의 지금'(D+ 히어로·작년 오늘·커플 캘린더) 컨텍스트와 맞다.
> Trip 7화면을 `HomeStackParamList` 로 옮기고 진입 표면을 3층으로 재구성:
> ① 홈의 조건부 D-day 카드(`TripPeek` — 여행 중 > 작년 오늘 > D-day 우선순위 슬롯),
> ② 커플 캘린더 그리드의 여행 기간 띠 + '우리 여행' 섹션(상세/전체 목록 진입),
> ③ 캘린더의 '여행 만들기' CTA(여행이 0개일 때의 유일한 생성 진입로).
> 딥링크 경로(`doubly://trips/*`)는 문자열 그대로 유지해 기존 링크가 계속 열린다.
> 백엔드·API·타입은 변경 없음.
>
> **후속 — 캘린더 기간 일정**: 캘린더가 기간(여행 틴트)을 다루게 되면서 일반 일정도
> 시작~종료 기간 등록을 지원한다(`couple_events.end_date`, V58 — NULL 이면 하루 일정).
> 월 조회는 겹침(overlap) 기준이라 전 달에 시작해 이 달로 이어지는 일정도 나타나고,
> 매년 반복 일정은 기간을 갖지 않으며(서비스 검증), D-day 푸시는 시작일 아침에만 울린다.
> 진행 중인 기간 일정은 목록에서 "진행 중" 배지로, upcoming 에서는 맨 앞으로 온다.
...
>
> **[후속 2026-09-14] 이 결정을 다시 뒤집는다 — 운동 탭 자리에 앨범.** 분리 근거였던
> "운동·식단은 독립적으로 자주 본다"는 전제가 9/9 운동·식단 사용성 분석으로 깨졌다(운동 탭은
> 구조적으로 저사용). 운동은 식단 탭 상단 체크인 카드로 흡수해 그 탭을 **럽바디**로 묶고, 빈 자리에
> 일상·식단·운동·맛집 4소스 사진 + 타임라인·여행·작년 오늘을 모은 **우리 탭**(앨범)을 넣는다.
> 확정 탭바 **홈 · 럽바디 · 채팅 · 우리 · 럽슐랭**. 세그먼트 토글로는 돌아가지 않는다. 근거·설계·구현
> 순서는 `docs/ALBUM_TAB_IA_2026-09-14.md`.
...
>
> **[후속 2026-08]** 이 탭이 이후 **럽슐랭**으로 리브랜딩되며 위 전제("여행을 가리면 안
> 되니 상위 개념 '장소'")가 뒤집혔다 — 정확히 이 문단이 경고한 "좁히기"가 실행된 셈이다.
> 그래서 여행 스위트는 이 탭에서 홈 스택으로 이관했다. 상세는 "커플 여행(Trip)" 절의
> "여행 IA 이관" 노트 참고.
